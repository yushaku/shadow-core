...
abstract contract Payments is RouterImmutables {
    ...
    function payPortion(
        ...
        if (bips == 0 || bips > FEE_BIPS_BASE) revert INVALID_BIPS();
        if (token == Constants.ETH) {
    ...
    function sweep(
        ...
        if (token == Constants.ETH) {
            ...
            if (balance < amountMinimum) revert INSUFFICIENT_ETH();
            if (balance > 0) recipient.safeTransferETH(balance);
        } else {
            balance = ERC20(token).balanceOf(address(this));
            if (balance < amountMinimum) revert INSUFFICIENT_TOKEN();
