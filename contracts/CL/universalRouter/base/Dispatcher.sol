			...
			if (command < Commands.SECOND_IF_BOUNDARY) {
				...
				if (command < Commands.FIRST_IF_BOUNDARY) {
					...
					if (command == Commands.V2_SWAP_EXACT_IN) {
						...
						IAllowanceTransfer.AllowanceTransferDetails[] memory batchDetails = abi
							.decode(inputs, (IAllowanceTransfer.AllowanceTransferDetails[]));
				...
				if (command < Commands.THIRD_IF_BOUNDARY) {
					if (command == Commands.SEAPORT_V1_5) {
						...
						if (success)
							ICryptoPunksMarket(CRYPTOPUNKS).transferPunk(map(recipient), punkId);
