...
contract RamsesV3Pool is IRamsesV3Pool {
	...
	constructor() {
		...
		(factory, token0, token1, $.fee, tickSpacing) = IRamsesV3PoolDeployer(msg.sender)
			.parameters();
			...
			if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
				...
				if (step.initialized) {
					...
					if (!cache.computedLatestObservation) {
						(cache.tickCumulative, cache.secondsPerLiquidityCumulativeX128) = Oracle
							.observeSingle(
								$.observations,
	...
	function collectProtocol(
		...
		require(
			msg.sender == IRamsesV3Factory(factory).feeCollector(),
			ProtocolActions.NOT_AUTHORIZED()
		);
