...
library Position {
	...
	function update(
		...
		if (
			checkpointLength == 0 ||
			$.positionCheckpoints[_positionHash][checkpointLength - 1].period != period
		) {
			$.positionCheckpoints[_positionHash].push(
		...
		int256 secondsDebtDeltaX96 = liquidityDelta > 0 /// @dev case: delta > 0
			? SafeCast.toInt256(
				/// @dev round upwards
	...
	function _updatePosition(
	...
	) external returns (PositionInfo storage position) {
		PoolStorage.PoolState storage $ = PoolStorage.getStorage();
		...
		if (params.liquidityDelta != 0) {
			...
			(int56 tickCumulative, uint160 secondsPerLiquidityCumulativeX128) = Oracle
				.observeSingle(
					$.observations,
					params._blockTimestamp,
					0,
	...
	function initializeSecondsStart(
		...
		if (position.liquidity > 0) {
			...
			position.periodRewardInfo[secondsInRangeParams.period].secondsDebtX96 = -int256(
