...
contract Router is IRouter {
	...
	function swapExactTokensForTokensSupportingFeeOnTransferTokens(
		...
		require(
			IERC20Extended(routes[routes.length - 1].to).balanceOf(to) - balanceBefore >=
				amountOutMin,
	...
	function swapExactETHForTokensSupportingFeeOnTransferTokens(
		...
		assert(
			IWETH(WETH).transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), msg.value)
		);
		uint256 balanceBefore = IERC20Extended(routes[routes.length - 1].to).balanceOf(to);
		_swapSupportingFeeOnTransferTokens(routes, to);
		require(
	...
	function _safeTransferFrom(address token, address from, address to, uint256 value) internal {
		require(token.code.length > 0);
		(bool success, bytes memory data) = token.call(
			abi.encodeWithSelector(IERC20Extended.transferFrom.selector, from, to, value)
		);
		require(success && (data.length == 0 || abi.decode(data, (bool))));
	}
}
