...
contract Voter is IVoter, ReentrancyGuard, Initializable {
	...
	function initialize(
		...
		/// @dev perma approval
		IERC20(ysk).approve(xy, type(uint256).max);
	}
	...
	function _distribute(address _gauge, uint256 _claimable, uint256 _period) internal {
		...
		if (isAlive[_gauge]) {
			...
			/// @dev _claimable could be 0 if emission is 100% xy
			if (_claimable > 0) {
			...
			if (_xShadowClaimable > 0) {
				if (
					...
					_xShadowClaimable < IGauge(_gauge).left(_xY)
				) {
					canDistribute = false;
				}
			}
			...
			if (canDistribute) {
				...
				if (_xShadowClaimable > 0) {
					/// @dev convert, then notify the xy
					IXY(_xY).convertEmissionsToken(_xShadowClaimable);
					IGauge(_gauge).notifyRewardAmount(_xY, _xShadowClaimable);
				}
	...
	function reviveGauge(address _gauge) public onlyGovernance {
		...
		if (isLegacyGauge[_gauge]) {
			address pool = poolForGauge[_gauge];
			address feeRecipient = IFeeRecipientFactory(feeRecipientFactory).feeRecipientForPair(
				pool
			);
	...
	function createGauge(address _pool) external returns (address) {
		...
		IERC20(xy).approve(_gauge, type(uint256).max);
	...
	function createCLGauge(
		...
		/// @dev unlimited approve shadow and xy to the gauge
		IERC20(ysk).approve(_gauge, type(uint256).max);
		IERC20(xy).approve(_gauge, type(uint256).max);
		/// @dev update mappings
	...
	function _claimablePerPeriod(address pool, uint256 period) internal view returns (uint256) {
		uint256 numerator = (totalRewardPerPeriod[period] * poolTotalVotesPerPeriod[pool][period]) *
			1e18;
