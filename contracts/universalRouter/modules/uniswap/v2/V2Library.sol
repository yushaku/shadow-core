...
library V2Library {
	...
	function getAmountOut(
		...
		if (stable) {
			...
			uint256 y = reserveOut -
				_get_y(amountIn + reserveIn, k, reserveOut, decimalsIn, decimalsOut);
	...
	function getAmountIn(
		...
		if (stable) {
			...
			uint256 y = _get_y(reserveOut - amountOut, k, reserveIn, decimalsIn, decimalsOut) -
				reserveIn;
	...
	function getAmountInMultihop(
		...
		for (uint256 i = path.length - 1; i > 0; i--) {
			...
			uint256 decimalsIn;
			uint256 decimalsOut;
			(pair, reserveIn, reserveOut, decimalsIn, decimalsOut) = pairAndReservesFor(
				factory,
				initCodeHash,
				path[i].from,
				path[i].to,
