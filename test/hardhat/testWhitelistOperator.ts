...
describe("WhitelistOperator", function () {
  ...
  it("Should allow xShadow whitelisting", async function () {
    await expect(c.commandCenter.whitelistAddress(randomAddress.getAddress())).to.not.be.reverted;
  ...
  it("Should allow removing xShadow whitelists", async function () {
    await expect(c.commandCenter.removeWhitelistedAddress(randomAddress.getAddress())).to.not.be
      .reverted;
  ...
  it("Should not fail when batch whitelisting / blacklisting", async function () {
    await expect(c.commandCenter.batchAddWhitelist([randomAddress.getAddress(), randomAddressTwo]))
      .to.not.be.reverted;
    expect(await c.xShadow.isWhitelisted(randomAddress.getAddress())).true;
    expect(await c.xShadow.isWhitelisted(randomAddressTwo)).true;
    await expect(
      c.commandCenter.batchRemoveWhitelist([randomAddress.getAddress(), randomAddressTwo]),
    ).to.not.be.reverted;
    expect(await c.xShadow.isWhitelisted(randomAddress.getAddress())).false;
  ...
  it("Should allow setting xShadow ratio", async function () {
    ...
    await expect(c.commandCenter.setDefaultRatio(ratio + 1n)).to.not.be.reverted;
    ratio = ratio + 1n;
  ...
  it("Should allow setting gaugeXRaRatios", async function () {
    ...
    await expect(c.commandCenter.setGaugeRatios([gauge.getAddress()], [ratio + 1n])).to.not.be
      .reverted;
    ratio = ratio + 1n;
  ...
  it("Should allow setting gaugeXRaRatios by pair address", async function () {
    let ratio = await c.voter.gaugeXRatio(gauge.getAddress());
    await expect(c.commandCenter.setGaugeRatiosByPair([pair.getAddress()], [ratio + 1n])).to.not.be
      .reverted;
    ratio = ratio + 1n;
    expect(await c.voter.gaugeXRatio(gauge.getAddress())).equal(ratio);
  });
  ...
  it("Should allow adding new operators", async function () {
    expect(await c.commandCenter.hasRole(operatorRole, randomAddress.getAddress())).false;
    await expect(c.commandCenter.grantRole(operatorRole, randomAddress.getAddress())).to.not.be
      .reverted;
    expect(await c.commandCenter.hasRole(operatorRole, randomAddress.getAddress())).true;
  });

  it("Should allow new operator to interact", async function () {
    // only testing one function
    await expect(c.commandCenter.connect(randomAddress).whitelistAddress(randomAddressTwo)).to.not
      .be.reverted;
  });

  it("Should not allow operator to grant roles", async function () {
    await expect(c.commandCenter.connect(randomAddress).grantRole(operatorRole, randomAddressTwo))
      .to.be.reverted;
  });

  it("Should not allow operator to interact when role is revoked", async function () {
    await expect(c.commandCenter.revokeRole(operatorRole, randomAddress.getAddress())).to.not.be
      .reverted;
    await expect(c.commandCenter.connect(randomAddress).removeWhitelistedAddress(randomAddressTwo))
      .to.be.reverted;
  });
  ...
  it("Should return the proper xRaRatioByGauge and pair", async function () {
    ...
    expect(await c.commandCenter.getRatioByGauge(gauge.getAddress())).equal(ratio);
    expect(await c.commandCenter.getRatioByPair(pair.getAddress())).equal(ratio);
  });
});
