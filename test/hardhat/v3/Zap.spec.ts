...
describe("Zap", () => {
  ...
  describe("#zap in", () => {
    it("addLiquidityAndStake", async () => {
      expect(await gauge.balanceOf(deployer), "there shouldn't be starting balance").eq(0n);
      expect(await pair.balanceOf(deployer), "there shouldn't be starting balance").eq(0n);
      ...
      expect(await pair.balanceOf(deployer), "balance shouldn't go to user").eq(0n);
      expect(await gauge.balanceOf(deployer), "user should have stakes").matchSnapshot();
    });
    it("addLiquidityETHAndStake", async () => {
      expect(await wethGauge.balanceOf(deployer), "there shouldn't be starting balance").eq(0n);
      expect(await wethPair.balanceOf(deployer), "there shouldn't be starting balance").eq(0n);
      ...
      expect(await wethPair.balanceOf(deployer), "balance shouldn't go to user").eq(0n);
      expect(await wethGauge.balanceOf(deployer), "user should have stakes").matchSnapshot();
    });
  });
});
