...
describe("RebaseDistributor", () => {
  ...
  describe("#initial states", () => {
    it("initial states", async () => {
      expect(await distributor.voter(), "voter address not right").equal(await voter.getAddress());
      expect(await distributor.votingEscrow(), "votingEscrow address not right").equal(
        await ve.getAddress(),
      );
      expect(await distributor.shadow(), "shadow address not right").equal(
        await c.shadow.getAddress(),
      );
      expect(await distributor.firstPeriod(), "firstPeriod address not right").equal(
        await voter.getPeriod(),
      );
      expect(
        await c.shadow.allowance(distributor, ve),
  ...
  describe("#_deposit", () => {
    it("normal operation", async () => {
      ...
      expect(await distributor.balanceOf(tokenId), "balanceOf not right").equal(votingPower);

      expect(await distributor.userVotingPower(period, tokenId), "userVotingPower not right").equal(
        votingPower,
      );

      expect(await distributor.votingPower(period), "votingPower not right").equal(votingPower);
    });
  ...
  describe("#_withdraw", () => {
    it("normal operation", async () => {
      ...
      expect(await distributor.balanceOf(tokenId), "balanceOf not right").equal(0n);

      expect(await distributor.userVotingPower(period, tokenId), "userVotingPower not right").equal(
        0n,
      );

      expect(await distributor.votingPower(period), "votingPower not right").equal(0n);
    });
  ...
  describe("#claimRebase", () => {
    ...
    it("normal operation", async () => {
      ...
      expect(await distributor.lastClaim(tokenId), "lastClaim wasn't recorded").equal(period);
    });
    ...
    it("shouldn't get anything if already claimed", async () => {
      ...
      expect(await ve.votingPower(tokenId), "rebase should be 0 after claiming").equal(
        votingPowerBefore,
      );
    });
    ...
    it("should be able to claim more if notify is called again", async () => {
      ...
      await c.shadow.approve(distributor, ethers.MaxUint256);
  ...
  describe("#claimPeriodRebase", () => {
    ...
    it("normal operation", async () => {
      ...
      expect(await distributor.lastClaim(tokenId), "lastClaim wasn't recorded").equal(period - 1n);
    });
    ...
    it("future periods shouldn't be claimed", async () => {
      const period = (await voter.getPeriod()) + 1n;
      // claim
      await expect(distributor.claimPeriodRebase(tokenId, period)).to.be.revertedWithCustomError(
        distributor,
        "VoteNotFinalized",
      );
    });
  ...
  describe("#earned", () => {
    ...
    it("after claiming", async () => {
      ...
      expect(await distributor.earned(tokenId), "earned should be 0 after claiming").equal(0n);
    });
    ...
    it("after claiming and notify is called again", async () => {
      ...
      expect(await distributor.earned(tokenId), "earned amount wasn't correct").equal(rebase);
    });
  });
  ...
  describe("#notifyRewardAmount", () => {
    it("normal operation", async () => {
      ...
      expect(await distributor.rewardSupply(period), "rewardSupply amount not correct").equal(
        rebase,
      );
    });
