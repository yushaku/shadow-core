...
describe("Voter", () => {
  ...
  async function createMorePools() {
    ...
    await c.pairFactory.createPair(await c.usdc.getAddress(), await c.dai.getAddress(), true);
  ...
  async function createClGauge(poolAddress: string): Promise<typechain.GaugeV3> {
    const pool = await ethers.getContractAt("RamsesV3Pool", poolAddress);
    const token0 = await pool.token0();
    const token1 = await pool.token1();
    const tickSpacing = await pool.tickSpacing();
    const gaugeAddress = await voter.createCLGauge.staticCall(token0, token1, tickSpacing);
    await voter.createCLGauge(token0, token1, tickSpacing);
    const gauge = await ethers.getContractAt("GaugeV3", gaugeAddress);
    return gauge;
  }

  async function washtradeForFees(token0: AddressLike, token1: AddressLike, legacy = false) {
    if (legacy) {
  ...
  async function batchGaugesFixture() {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      await c.pairFactory.createPair(await tokenA.getAddress(), await tokenB.getAddress(), true);
      await voter.createGauge(pairAddress);
    }
  ...
  describe("#initialization", () => {
    it("initialization", async () => {
      ...
      expect(await voter.shadow(), "emission token address").equal(await c.shadow.getAddress());
      ...
      expect(await voter.feeDistributorFactory(), "feeDistributorFactory address").equal(
        await c.feeDistributorFactory.getAddress(),
      );
      expect(await voter.minter(), "minter address").equal(await c.minter.getAddress());
      expect(await voter.governor(), "governor address").equal(deployer.address);
      expect(await voter.pairFeeFactory(), "pairFeeFactory address").equal(
        await c.pairFeeFactory.getAddress(),
      );
      expect(await voter.votingEscrow(), "votingEscrow address").equal(
        await c.votingEscrow.getAddress(),
      );
      expect(await voter.clFactory(), "clFactory address").equal(await c.factory.getAddress());
      expect(await voter.clGaugeFactory(), "clGaugeFactory address").equal(
        await c.gaugeV3Factory.getAddress(),
      );
      expect(await voter.nfpManager(), "nfpManager address").equal(await c.nfpManager.getAddress());
      expect(await voter.rebaseDistributor(), "rebaseDistributor address").equal(
        await c.rebaseDistributor.getAddress(),
      );
    });
  });
  ...
  describe("#setGovernor", () => {
    it("set with deployer", async () => {
      ...
      expect(await voter.governor(), "governor not changed").equal(MULTISIG);
    });
    it("set with attacker", async () => {
      await expect(voter.connect(attacker).setGovernor(MULTISIG)).to.be.revertedWithCustomError(
        voter,
  ...
  describe("#whitelist", () => {
    it("set with deployer", async () => {
      ...
      expect(await voter.isWhitelisted(MULTISIG), "whitelist not changed").equal(true);

      // whitelist existing token
      await expect(voter.whitelist(MULTISIG)).to.be.revertedWithCustomError(
        voter,
        "AlreadyWhitelisted",
      );
    });
    it("set with attacker", async () => {
      await expect(voter.connect(attacker).whitelist(MULTISIG)).to.be.revertedWithCustomError(
        voter,
        "AccessManagedUnauthorized",
      );
    });
  ...
  describe("#forbid", () => {
    it("set with deployer", async () => {
      ...
      expect(await voter.isForbidden(MULTISIG), "isForbidden not changed").equal(true);

      // remove forbid
      await voter.whitelist(MULTISIG);
      expect(await voter.isForbidden(MULTISIG), "isForbidden not changed").equal(false);
    });
    it("set with attacker", async () => {
      await expect(voter.connect(attacker).forbid(MULTISIG)).to.be.revertedWithCustomError(
        voter,
        "AccessManagedUnauthorized",
      );
    });
  ...
  describe("#killGauge", () => {
    ...
    describe("Normal case - legacy gauges", () => {
      ...
      afterEach("Try voting", async () => {
        // get some votes in
        await voter.vote(c.tokenId, [c.pair.getAddress(), c.clPool.getAddress()], [1, 1]);
        // increase a week and advance period
        await helpers.time.increase(86400 * 7);
        await c.minter.updatePeriod();
        const period = await voter.getPeriod();
        expect({
          pairVotes: await voter.poolTotalVotesPerPeriod(c.pair.getAddress(), period),
          clVotes: await voter.poolTotalVotesPerPeriod(c.clPool.getAddress(), period),
        }).to.matchSnapshot("votes not matching snapshot");
      });

      it("no votes on pool", async () => {
        const tokensBefore = await c.shadow.balanceOf(deployer.address);

        // kill gauge
        await voter.killGauge(c.gauge.getAddress());
        expect(await voter.lastDistro(c.gauge.getAddress()), "lastDistro wrong").eq(
          await voter.getPeriod(),
        );
        expect(await c.shadow.balanceOf(deployer.address), "there should be no increase").eq(
          tokensBefore,
        );
        expect(await voter.isAlive(c.gauge.getAddress()), "gauge still alive").eq(false);
      });
      ...
      it("some votes on pool", async () => {
        ...
        expect(await c.shadow.balanceOf(deployer.address)).to.matchSnapshot(
          "tokens should be sent to governor",
        );
        expect(await voter.isAlive(c.gauge.getAddress()), "gauge still alive").eq(false);
      });
      ...
      it("FeeSplitWhenNoGauge == true", async () => {
        ...
        // kill gauge
        await voter.killGauge(c.gauge.getAddress());
        expect(await c.pair.pairFees(), "pairFees wrong").eq(deployer.address);
      });
      ...
      it("FeeSplitWhenNoGauge == false", async () => {
        ...
        // kill gauge
        await voter.killGauge(c.gauge.getAddress());
        expect(await c.pair.pairFees(), "pairFees wrong").eq(ethers.ZeroAddress);
      });
    });
  });
  ...
  describe("#reviveGauge", () => {
    ...
    describe("Revive killed gauges", () => {
      beforeEach("kill some gauges", async () => {
        // get some votes in
        await voter.vote(c.tokenId, [c.clPool.getAddress(), c.pair.getAddress()], [1, 1]);
        // increase a week and advance period
        await helpers.time.increase(86400 * 7);
        await c.minter.updatePeriod();
      ...
      it("revive legacy gauge", async () => {
        ...
        expect(await voter.isAlive(c.gauge.getAddress()), "gauge not revived").equal(true);
        expect(await c.pair.pairFees(), "pairFees not updated").equal(
          await c.pairFeeFactory.pairFeesForPair(c.pair.getAddress()),
        );
        const period = await voter.getPeriod();
        expect(await voter.lastDistro(c.gauge.getAddress()), "lastDistro not updated").equal(
          period,
        );
      });
      ...
      it("revive cl gauge", async () => {
        ...
        expect(await voter.isAlive(c.clGauge.getAddress()), "gauge not revived").equal(true);
        const period = await voter.getPeriod();
        expect(await voter.lastDistro(c.clGauge.getAddress()), "lastDistro not updated").equal(
          period,
        );
      });
    });
  ...
  describe("#stuckEmissionsRecovery", () => {
    ...
    describe("#recovery on killed gauges", () => {
      ...
      it("normal operation", async () => {
        const period = await voter.getPeriod();
        const balanceBefore = await c.shadow.balanceOf(voter.getAddress());

        // recover
        await voter.stuckEmissionsRecovery(c.gauge.getAddress(), period);

        const balanceAfter = await c.shadow.balanceOf(voter.getAddress());

        expect(balanceAfter, "voter didn't send tokens on recovery").lt(balanceBefore);

        expect(
          await voter.gaugePeriodDistributed(c.gauge.getAddress(), period),
        ...
        expect(await c.shadow.balanceOf(voter.getAddress()), "voter sent tokens after recovery").eq(
          balanceAfter,
        );
      });
      ...
      it("calling on future period should have no effect", async () => {
        ...
        expect(balanceAfter, "voter sent tokens when it's not supposed to").eq(balanceBefore);
      ...
      it("should revert for attacker", async () => {
        const period = await voter.getPeriod();
        await expect(
          voter.connect(attacker).stuckEmissionsRecovery(c.gauge.getAddress(), period),
  ...
  describe("#whitelistGaugeRewards", () => {
    it("normal operation on gauges", async () => {
      await voter.whitelistGaugeRewards(c.gauge.getAddress(), c.dai.getAddress());

      // should be able to add dai as reward to the gauge
      await c.dai.approve(c.gauge.getAddress(), ethers.MaxUint256);
      await c.gauge.notifyRewardAmount(c.dai.getAddress(), ethers.parseEther("100"));
    });

    it("normal operation on cl gauges", async () => {
      await voter.whitelistGaugeRewards(c.clGauge.getAddress(), c.dai.getAddress());

      // should be able to add dai as reward to the gauge
      await c.dai.approve(c.clGauge.getAddress(), ethers.MaxUint256);
      await c.clGauge.notifyRewardAmount(c.dai.getAddress(), ethers.parseEther("100"));
    });

    it("should revert for attacker", async () => {
      await expect(
        voter.connect(attacker).whitelistGaugeRewards(c.gauge.getAddress(), c.dai.getAddress()),
      ).to.be.revertedWithCustomError(voter, "AccessManagedUnauthorized");
    });
  });
  ...
  describe("#removeGaugeRewardWhitelist", () => {
    ...
    it("normal operation on gauges", async () => {
      await voter.removeGaugeRewardWhitelist(c.gauge.getAddress(), c.dai.getAddress());

      // should not be able to add dai as reward to the gauge
      await c.dai.approve(c.gauge.getAddress(), ethers.MaxUint256);
      await expect(
        c.gauge.notifyRewardAmount(c.dai.getAddress(), ethers.parseEther("100")),
      ).to.be.revertedWithCustomError(c.gauge, "NotWhitelisted");
    });

    it("normal operation on cl gauges", async () => {
      await voter.removeGaugeRewardWhitelist(c.clGauge.getAddress(), c.dai.getAddress());
    ...
    it("should revert for attacker", async () => {
      await expect(
        voter
          .connect(attacker)
          .removeGaugeRewardWhitelist(c.gauge.getAddress(), c.usdc.getAddress()),
      ).to.be.revertedWithCustomError(voter, "AccessManagedUnauthorized");
    });
  });
  ...
  describe("#setMainCurve", () => {
    ...
    beforeEach("deploy another pool of the same pair", async () => {
      ...
      await c.pairFactory.createPair(await c.usdc.getAddress(), await c.usdt.getAddress(), false);
    ...
    it("initial state", async () => {
      expect(await voter.isAlive(gauge), "previous gauge should be alive").true;
      expect(await voter.isAlive(otherGauge), "other gauge shouldn't be alive").false;

      expect(await voter.poolRedirect(pair), "previous pool shouldn't be redirected").eq(
        ethers.ZeroAddress,
      );
      expect(await voter.poolRedirect(otherPair), "new pool should be redirected").eq(
    ...
    it("normal operation", async () => {
      ...
      expect(await voter.isAlive(gauge), "previous gauge shouldn't be alive").false;
      expect(await voter.isAlive(otherGauge), "other gauge should be alive").true;

      expect(await voter.poolRedirect(pair), "previous pool should be redirected").eq(
        await otherPair.getAddress(),
      );
      expect(await voter.poolRedirect(otherPair), "new pool should be directed to itself").eq(
        await otherPair.getAddress(),
      );
      ...
      expect(await voter.poolTotalVotesPerPeriod(pair, period), "votes shouldn't have changed").eq(
        await voter.totalVotesPerPeriod(period),
      );
      ...
      await pair.transfer(attacker.address, await pair.balanceOf(deployer.address));

      const pairFeesAddress = await pair.pairFees();

      expect(await pair.balanceOf(deployer.address), "no fees should be in pair fees yet").eq(0n);
    ...
    it("should revert for attacker", async () => {
      await expect(
        voter.connect(attacker).setMainCurve(c.usdc.getAddress(), c.usdt.getAddress(), false),
      ).to.be.revertedWithCustomError(voter, "AccessManagedUnauthorized");
    });
  });
  ...
  describe("#setMainTickSpacing", () => {
    ...
    beforeEach("deploy another pool of the same pair", async () => {
      ...
      otherClPool = await ethers.getContractAt("RamsesV3Pool", clPoolAddress);
    ...
    it("initial state", async () => {
      expect(await voter.isAlive(clGauge), "previous gauge should be alive").true;
      expect(await voter.isAlive(otherClGauge), "other gauge shouldn't be alive").false;

      expect(await voter.poolRedirect(clPool), "previous pool shouldn't be redirected").eq(
        ethers.ZeroAddress,
      );
      expect(await voter.poolRedirect(otherClPool), "new pool should be redirected").eq(
        await clPool.getAddress(),
      );
    ...
    it("normal operation", async () => {
      ...
      // change main tick spacing
      await voter.setMainTickSpacing(c.usdc, c.usdt, TICK_SPACINGS.NORMAL);

      expect(await voter.isAlive(clGauge), "previous gauge shouldn't be alive").false;
      expect(await voter.isAlive(otherClGauge), "other gauge should be alive").true;

      expect(await voter.poolRedirect(clPool), "previous pool should be redirected").eq(
        await otherClPool.getAddress(),
      );
      expect(await voter.poolRedirect(otherClPool), "new pool should be directed to itself").eq(
        await otherClPool.getAddress(),
      );
      ...
      expect(await c.usdc.balanceOf(otherFeeDist), "no fees should be in fee dist yet").eq(0n);
    ...
    it("should revert for attacker", async () => {
      await expect(
        voter
          .connect(attacker)
          .setMainTickSpacing(c.usdc.getAddress(), c.usdt.getAddress(), TICK_SPACINGS.NORMAL),
      ).to.be.revertedWithCustomError(voter, "AccessManagedUnauthorized");
    });
  });
  ...
  describe("#vote, poke, reset", () => {
    afterEach("Check data and try poke, reset votes", async () => {
      const period = (await voter.getPeriod()) + 1n;
      expect({
        votingPowerUsed: await voter.tokenIdVotingPowerPerPeriod(c.tokenId, period),
        totalVotesPerPeriod: await voter.totalVotesPerPeriod(period),
        votes: {
          clPool: await voter.poolTotalVotesPerPeriod(c.clGauge.getAddress(), period),
          pair: await voter.poolTotalVotesPerPeriod(c.gauge.getAddress(), period),
        },
      }).to.matchSnapshot("Votes not matching snapshot");
    ...
    it("voting for duplicate pools shouldn't cause problems", async () => {
      await voter.vote(c.tokenId, [c.clPool.getAddress(), c.clPool.getAddress()], [1, 1]);
    });
    ...
    it("voting for pools without an active gauge should record the right totals", async () => {
      ...
      await voter.vote(
        c.tokenId,
        [c.clPool.getAddress(), c.pair.getAddress(), ethers.ZeroAddress],
        [2, 1, 1],
      );
    });
    ...
    it("mismatched lengths", async () => {
      await expect(
        voter.vote(c.tokenId, [c.clPool.getAddress(), c.pair.getAddress()], [1]),
  ...
  describe("#createGauge, createCLGauge", () => {
    ...
    it("#createGauge", async () => {
      const gaugeAddress = await c.voter.createGauge.staticCall(pair.getAddress());
      await c.voter.createGauge(pair.getAddress());
      gauge = await ethers.getContractAt("Gauge", gaugeAddress);

      expect(await voter.isGauge(gauge.getAddress()), "isGauge not right").equal(true);
      const pairFees = await ethers.getContractAt(
        "PairFees",
        await c.pairFeeFactory.pairFeesForPair(pair.getAddress()),
      );
      expect(await pair.pairFees(), "pairFees not set").equal(await pairFees.getAddress());
      expect(await pairFees.feeDistributor(), "feeDistributor not set").equal(
        await voter.feeDistributorForGauge(gauge.getAddress()),
      );
      expect(
        await c.shadow.allowance(voter.getAddress(), gauge.getAddress()),
      ...
      expect(await voter.gaugeForPool(pair.getAddress()), "gaugeForPool not recorded").eq(
        await gauge.getAddress(),
      );
      expect(await voter.poolForGauge(gauge.getAddress()), "poolForGauge not recorded").eq(
        await pair.getAddress(),
      );
      expect(await voter.isAlive(gauge.getAddress()), "gauge not alive").eq(true);
      expect(await voter.isGauge(gauge.getAddress()), "gauge not recorded").eq(true);
      expect(
        await voter.isFeeDistributor(await voter.feeDistributorForGauge(gauge.getAddress())),
        "isFeeDistributor not recorded",
      ).eq(true);
      expect(await voter.isLegacyGauge(gauge.getAddress()), "isLegacyGauge not recorded").eq(true);
      expect(await voter.lastDistro(gauge.getAddress()), "lastDistro not recorded").not.equal(0n);
      expect(await voter.isClGauge(gauge.getAddress()), "isClGauge wrong").eq(false);
    });
    ...
    it("#createCLGauge", async () => {
      ...
      await c.voter.createCLGauge(c.usdc.getAddress(), c.dai.getAddress(), TICK_SPACINGS.STABLE);
      clGauge = await ethers.getContractAt("GaugeV3", clGaugeAddress);

      expect(
        await c.shadow.allowance(voter.getAddress(), clGauge.getAddress()),
        "emissions approval not set",
      ).gt(0);
      expect(await voter.gaugeForPool(clPool.getAddress()), "gaugeForPool not recorded").eq(
        await clGauge.getAddress(),
      );
      expect(await voter.poolForGauge(clGauge.getAddress()), "poolForGauge not recorded").eq(
        await clPool.getAddress(),
      );
      expect(await voter.isAlive(clGauge.getAddress()), "gauge not alive").eq(true);
      expect(await voter.lastDistro(clGauge.getAddress()), "lastDistro not recorded").not.equal(0n);
      expect(await voter.isGauge(clGauge.getAddress()), "isGauge not right").equal(true);
      expect(
      ...
      expect(
        await voter.isFeeDistributor(await voter.feeDistributorForGauge(clGauge.getAddress())),
        "isFeeDistributor not recorded",
      ).eq(true);
      expect(await voter.isClGauge(clGauge.getAddress()), "isClGauge not recorded").eq(true);
      expect(await voter.isLegacyGauge(clGauge.getAddress()), "isLegacyGauge wrong").eq(false);
    });

    it("#createCLGauge not pool", async () => {
      await expect(
        c.voter.createCLGauge(c.usdc.getAddress(), ethers.ZeroAddress, TICK_SPACINGS.STABLE),
        "non-pool not reverted",
      ).to.be.revertedWithCustomError(voter, "NotPool");
    });
    ...
    it("#createCLGauge existing gauge", async () => {
      await expect(
        c.voter.createCLGauge(c.usdc.getAddress(), c.usdt.getAddress(), TICK_SPACINGS.STABLE),
        "existing gauge not reverted",
      ).to.be.revertedWithCustomError(voter, "ActiveGauge");
    });
    ...
    it("#createCLGauge IsForbidden", async () => {
      ...
      await expect(
        c.voter
          .connect(attacker)
          .createCLGauge(c.usdc.getAddress(), c.dai.getAddress(), TICK_SPACINGS.STABLE),
    ...
    it("#createCLGauge NotWhitelisted", async () => {
      ...
      await expect(
        c.voter
          .connect(attacker)
          .createCLGauge(c.mim.getAddress(), c.dai.getAddress(), TICK_SPACINGS.STABLE),
        "NotWhitelisted not reverted",
      ).to.be.revertedWithCustomError(voter, "NotWhitelisted");

      // should still work for governor
      await c.voter.createCLGauge(c.mim.getAddress(), c.dai.getAddress(), TICK_SPACINGS.STABLE);
    });
  });
  ...
  describe("#claimClGaugeRewards, claimIncentives, claimRewards", () => {
    ...
    beforeEach("get some rewards into gauges", async () => {
      ...
      // create more pools
      ({ pair: otherPair, clPool: otherClPool } = await createMorePools());
      ...
      // add custom rewards
      await voter.whitelistGaugeRewards(clGauge.getAddress(), c.mim.getAddress());
      await voter.whitelistGaugeRewards(gauge.getAddress(), c.mim.getAddress());
      await c.clGauge.notifyRewardAmount(c.mim.getAddress(), ethers.parseEther("100"));
      await c.gauge.notifyRewardAmount(c.mim.getAddress(), ethers.parseEther("100"));
    ...
    it("#claimClGaugeRewards", async () => {
      ...
      const ramBalanceBefore = await c.shadow.balanceOf(deployer.address);
      ...
      const ramBalance = await c.shadow.balanceOf(deployer.address);
    ...
    it("#claimIncentives", async () => {
      ...
      const otherPairBalanceBefore = await otherPair.balanceOf(deployer.address);
      ...
      for (let i = 0; i < gauges.length; i++) {
        feeDistributors.push(await voter.feeDistributorForGauge(gauges[i].getAddress()));
      }
      ...
      await voter.claimIncentives(tokenId, feeDistributors, [tokens, tokens, tokens, tokens]);

      const pairBalance = await pair.balanceOf(deployer.address);
      const otherPairBalance = await otherPair.balanceOf(deployer.address);
  ...
  describe("#notifyRewardAmount", () => {
    it("should revert other than for minter", async () => {
      ...
      await expect(voter.notifyRewardAmount(1n)).to.be.revertedWithCustomError(
        voter,
        "Unauthorized",
      );
      ...
      const minterSigner = await ethers.getSigner(await c.minter.getAddress());
      await helpers.setBalance(await c.minter.getAddress(), ethers.parseEther("1000"));

      // add some emission tokens
      await c.shadow.transfer(c.minter.getAddress(), 1n);
      await c.shadow.connect(minterSigner).approve(voter.getAddress(), 1n);
  ...
  describe("#distribute, distributeForPeriod", () => {
    ...
    it("normal operation on distribute for legacy gauge", async () => {
      ...
      expect(await voter.lastDistro(c.gauge.getAddress()), "lastDistro not updated").equal(period);
      expect(
        await voter.gaugePeriodDistributed(c.gauge.getAddress(), period),
        "gaugePeriodDistributed not updated",
      ).equal(true);
      expect(await c.pair.balanceOf(feeDistributor), "fee didn't get distributed").gt(feeBefore);
      expect(
    ...
    it("normal operation on distribute for cl gauge", async () => {
      ...
      expect(
        await voter.gaugePeriodDistributed(c.clGauge.getAddress(), period),
        "gaugePeriodDistributed not updated",
      ).equal(true);
    ...
    it("normal operation on distributeForPeriod", async () => {
      ...
      // distribute
      await voter.distributeForPeriod(c.clGauge.getAddress(), period - 1n);
      ...
      expect(await voter.lastDistro(c.clGauge), "lastDistro should not update").equal(lastDistro);
    });
    ...
    it("distribute should call Minter.updatePeriod() if needed", async () => {
      ...
      expect(await c.minter.activePeriod(), "minter not updated").not.eq(activePeriodBefore);
    });
    ...
    it("distribute should call Pool._advancePeriod() if needed", async () => {
      ...
      expect(await c.clPool.lastPeriod(), "clPool not updated").not.eq(poolPeriodBefore);
    });
  });
  ...
  describe("#view functions", () => {
    ...
    it("#getAllPools", async () => {
      ...
      assert(allPools.includes(await c.pair.getAddress()), "pair not in allPools");
      assert(allPools.includes(await c.clPool.getAddress()), "clPool not in allPools");
    });
    ...
    it("#getAllGauges", async () => {
      ...
      assert(allGauges.includes(await c.gauge.getAddress()), "gauge not in allGauges");
      assert(allGauges.includes(await c.clGauge.getAddress()), "clGauge not in allGauges");
    });

    it("#getAllFeeDistributors", async () => {
      const allFeeDistributors = await voter.getAllFeeDistributors();

      expect(allFeeDistributors.length, "getAllFeeDistributors length not right").eq(2);
      assert(
        allFeeDistributors.includes(await c.voter.feeDistributorForGauge(c.gauge)),
        "gauge not in allFeeDistributors",
      );
      assert(
        allFeeDistributors.includes(await c.voter.feeDistributorForGauge(c.clGauge)),
    ...
    it("#isGauge", async () => {
      ...
      assert(await voter.isGauge(c.clGauge), "isGauge for clGauge not right");
    });

    it("#isFeeDistributor", async () => {
      assert(
        await voter.isFeeDistributor(await voter.feeDistributorForGauge(c.gauge)),
        "isFeeDistributor for gauge not right",
      );
      assert(
        await voter.isFeeDistributor(await voter.feeDistributorForGauge(c.clGauge)),
  ...
  describe("#edge cases", () => {
    describe("#Killed then revived gauge shouldn't get both distribute and recovery", () => {
      it("distribute first", async () => {
        ...
        expect(balanceAfter, "balance change after distribute").lt(balanceBefore);
    ...
    it.skip("if there isn't enough claimable to distribute", async () => {
      ...
      const tokenId = await c.votingEscrow.createLock.staticCall(emissions, deployer.address);
      ...
      const balanceBefore = await c.shadow.balanceOf(c.gauge.getAddress());
      await voter.distribute(c.gauge.getAddress());
      expect(
    ...
    it.skip("if a week of distribute is missed all distribute should happen at once", async () => {
      ...
      expect(
        (await voter.gaugePeriodDistributed(c.gauge.getAddress(), period)) &&
          (await voter.gaugePeriodDistributed(c.gauge.getAddress(), period - 1n)),
    ...
    it("delegate should reset on transfer", async () => {
      ...
      const delegateeBefore = await c.votingEscrow.idToDelegate(c.tokenId);

      expect(delegateeBefore).eq(await c.minter.getAddress(), "delegate not working");
      ...
      expect(delegatee).eq(ethers.ZeroAddress, "delegate not reset after transfer");
    });
    ...
    it("test router getAmountsIn with swapExactTokensForTokens", async () => {
      ...
      const amountOut = (await c.usdt.balanceOf(deployer.address)) - balanceBefore;
    ...
    it("test router getAmountsIn with swapTokensForExactTokens", async () => {
      ...
      const usdtPaid = balanceBefore - (await c.usdc.balanceOf(deployer.address));
