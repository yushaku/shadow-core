  ...
  for (const feeProtocol of [0, 6]) {
    describe(feeProtocol > 0 ? "fee is on" : "fee is off", () => {
      ...
      const gasTestFixture = async () => {
        ...
        await mint(wallet.address, 0n, minTick, maxTick, expandTo18Decimals(2n));
      ...
      describe("#swapExact0For1", () => {
        it("first swap in block with no tick movement", async () => {
          await snapshotGasCost(swapExact0For1(2000n, wallet.address));
          ...
          describe(description, () => {
            it("new position mint first in range", async () => {
              await snapshotGasCost(
                mint(wallet.address, 0n, tickLower, tickUpper, expandTo18Decimals(1n)),
              );
            });
            it("add to position existing", async () => {
              await mint(wallet.address, 0n, tickLower, tickUpper, expandTo18Decimals(1n));
              await snapshotGasCost(
                mint(wallet.address, 0n, tickLower, tickUpper, expandTo18Decimals(1n)),
              );
            });
            it("second position in same range", async () => {
              await mint(wallet.address, 1n, tickLower, tickUpper, expandTo18Decimals(1n));
              await snapshotGasCost(
                mint(other.address, 0n, tickLower, tickUpper, expandTo18Decimals(1n)),
              );
            });
        ...
        for (const { description, tickLower, tickUpper } of [
          ...
          describe(description, () => {
            ...
            beforeEach("mint a position", async () => {
              await mint(wallet.address, 0n, tickLower, tickUpper, liquidityAmount);
            });

            it("burn when only position using ticks", async () => {
              await snapshotGasCost(pool.burn(0n, tickLower, tickUpper, expandTo18Decimals(1n)));
            });
            it("partial position burn", async () => {
              await snapshotGasCost(
                pool.burn(0n, tickLower, tickUpper, expandTo18Decimals(1n) / 2n),
              );
            });
            it("entire position burn but other positions are using the ticks", async () => {
              await mint(other.address, 0n, tickLower, tickUpper, expandTo18Decimals(1n));
              await snapshotGasCost(pool.burn(0n, tickLower, tickUpper, expandTo18Decimals(1n)));
            });
      ...
      describe("#poke", () => {
        ...
        it("best case", async () => {
          ...
          await swapExact0For1(expandTo18Decimals(1n) / 100n, wallet.address);
          await snapshotGasCost(pool.burn(0n, tickLower, tickUpper, 0));
        });
      });
      ...
      describe("#collect", () => {
        ...
        it("close to worst case", async () => {
          ...
          await snapshotGasCost(
            pool.collect(wallet.address, 0n, tickLower, tickUpper, MaxUint128, MaxUint128),
          );
        });
      });
