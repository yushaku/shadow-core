...
async function tokensFixture(): Promise<TokensFixture> {
  const tokenFactory = await ethers.getContractFactory("contracts/v2/test/TestERC20.sol:TestERC20");
...
type TokensAndFactoryFixture = FactoryFixture & TokensFixture & TaxTokensFixture;
...
interface PoolFixture extends TokensAndFactoryFixture {
  ...
  createGauge(poolAddress: string): Promise<{ gauge: GaugeV2; feeDistributor: FeeDistributor }>;
  c: TestDeploy;
}
...
export const SECONDS_PER_LIQUIDITY_INIT = "545100501377799618628145949242437061247919718400"; // 1601906400 * 2 ** 128
export const SECONDS_PER_LIQUIDITY_PERIOD_INIT = "544965749560498926996614452897894081036183142400"; // 1601510400 * 2 ** 128
...
export async function poolFixture(): Promise<PoolFixture> {
  ...
  const MockTimeProtocolActions = await ethers.getContractFactory("MockTimeProtocolActions");
  ...
  const MockTimePosition = await ethers.getContractFactory("MockTimePosition", {
  ...
  const MockTimeClPoolFactory = await ethers.getContractFactory("MockTimeClPoolFactory");
  ...
  const factoryProxy = await ethers.getContractAt("TransparentUpgradeableProxy", c.factory.address);
  ...
  await factoryProxy.connect(proxyAdmin).upgradeTo(mockTimeClPoolFactory.address);

  const mockTimeClPool = await MockTimeClPool.deploy();

  const factory = MockTimeClPoolFactory.attach(factoryProxy.address) as MockTimeClPoolFactory;
  ...
  const nftProxy = await ethers.getContractAt("TransparentUpgradeableProxy", c.nfpManager.address);

  const PositionManagerAux = await ethers.getContractFactory("PositionManagerAux");
  ...
  const MockNfpManager = await ethers.getContractFactory("MockTimeNonfungiblePositionManager", {
    libraries: {
  ...
  const mockNftImp = (await MockNfpManager.deploy()) as MockTimeNonfungiblePositionManager;
  ...
  const deployerDeployedLength = (await contractDeployer.deployedContractsLength()).toNumber();
  const salts = [];
  const newTestContractsLength = 2;
  for (let i = deployerDeployedLength; i < deployerDeployedLength + newTestContractsLength; i++) {
    salts.push(deployerDeployedLength + i);
  }

  await contractDeployer.deployMany(TransparentUpgradeableProxy__factory.bytecode, salts);

  const proxiesAddresses = (await contractDeployer.getDeployedContracts()).slice(
  ...
  for (let i = 0; i < newTestContractsLength; i++) {
    proxies.push(await ethers.getContractAt("TransparentUpgradeableProxy", proxiesAddresses[i]));
  }

  // Deploy and attach the new test contracts to the proxies

  const calleeContractFactory = await ethers.getContractFactory("TestClPoolCallee");
  const routerContractFactory = await ethers.getContractFactory("TestClPoolRouter");

  const swapTargetCalleeImp = (await calleeContractFactory.deploy()) as TestClPoolCallee;
  const swapTargetRouterImp = (await routerContractFactory.deploy()) as TestClPoolRouter;

  await proxies[0].initialize(swapTargetCalleeImp.address, c.proxyAdmin.address, "0x");
  await proxies[1].initialize(swapTargetRouterImp.address, c.proxyAdmin.address, "0x");

  const swapTargetCallee = await ethers.getContractAt("TestClPoolCallee", proxies[0].address);
  const swapTargetRouter = await ethers.getContractAt("TestClPoolRouter", proxies[1].address);
  ...
  return {
    ...
    createPool: async (
      ...
      const tx = await factory["createPool(address,address,uint24,int24,uint160)"](
        firstToken.address,
        secondToken.address,
        fee,
        tickSpacing,
    ...
    createNormalPool: async (fee, sqrtPriceX96, firstToken = token0, secondToken = token1) => {
      const tx = await factory["createPool(address,address,uint24,uint160)"](
        firstToken.address,
        secondToken.address,
        fee,
        sqrtPriceX96,
    ...
    createGauge: async (poolAddress: string) => {
      ...
      const pool = await ethers.getContractAt("MockTimeClPool", poolAddress);
      ...
      const gauge = (await ethers.getContractAt("GaugeV2", gaugeAddress)) as GaugeV2;
      const feeDistributorAddress = await c.voter.feeDistributors(gauge.address);
