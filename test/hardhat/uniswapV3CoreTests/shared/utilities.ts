...
// bigint only truncates toward zero, negative values will be incorrect if this is not used
function floorDiv(x: bigint, y: bigint): bigint {
  const result = x / y;
  return x < 0n && y * result !== x ? result - 1n : result;
}

function ceilDiv(x: bigint, y: bigint): bigint {
  return x > 0n && y > 0n && x % y !== 0n ? x / y + 1n : x / y;
}
...
export const TICK_SPACINGS: { [amount in FeeAmount]: bigint } = {
  [FeeAmount.LOW]: 10n,
  [FeeAmount.MEDIUM]: 60n,
  [FeeAmount.HIGH]: 200n,
};
...
export function createPoolFunctions({
  ...
  const mintViaNFP: MintNfpFunction = async (
    ...
    await token0.approve(nfpManager.getAddress(), ethers.MaxUint256);
    await token1.approve(nfpManager.getAddress(), ethers.MaxUint256);

    const poolFee = await pool.fee();

    const mintParams = {
      token0: token0.getAddress(),
      token1: token1.getAddress(),
      fee: poolFee,
      tickLower,
      tickUpper,
  ...
  const flash: FlashFunction = async (
    ...
    return swapTarget.flash(
      pool.getAddress(),
      typeof to === "string" ? to : to.getAddress(),
      amount0,
      amount1,
      pay0,
      pay1
