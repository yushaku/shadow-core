...
describe("UniswapV3Pool", () => {
  ...
  it("code size within limit", async () => {
    ...
    hre.config.contractSizer.only = ["UniswapV3Pool"];
    ...
    function parseFileSize(text: string): number {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith("|  UniswapV3Pool ")) {
  ...
  describe("#initialize", () => {
    it("fails if already initialized", async () => {
      ...
      await expect(pool.initialize(encodePriceSqrt(1n, 1n).toString())).to.be.reverted;
    });
    it("fails if starting price is too low", async () => {
      await expect(pool.initialize(1n)).to.be.revertedWithCustomError(pool, "R");
    ...
    it("initializes the first observations slot", async () => {
      ...
      checkObservationEquals(await pool.observations(0), {
        secondsPerLiquidityCumulativeX128: SECONDS_PER_LIQUIDITY_PERIOD_INIT,
        initialized: true,
  ...
  describe("#flash", () => {
    ...
    it("fails if no liquidity", async () => {
      await pool.initialize(encodePriceSqrt(1n, 1n).toString());
      await expect(flash(100n, 200n, other.address)).to.be.revertedWithCustomError(pool, "L");
      await expect(flash(100n, 0n, other.address)).to.be.revertedWithCustomError(pool, "L");
      await expect(flash(0n, 200n, other.address)).to.be.revertedWithCustomError(pool, "L");
    });
    describe("after liquidity added", () => {
      ...
      describe("fee off", () => {
        ...
        it("fails if original balance not returned in either token", async () => {
          await expect(flash(1000n, 0n, other.address, 999n, 0n)).to.be.reverted;
          await expect(flash(0n, 1000n, other.address, 0n, 999n)).to.be.reverted;
        });
        it("fails if underpays either token", async () => {
          await expect(flash(1000n, 0n, other.address, 1002n, 0n)).to.be.reverted;
          await expect(flash(0n, 1000n, other.address, 0n, 1002n)).to.be.reverted;
        });
        ...
        it("allows donating token0 and token1 together", async () => {
          await expect(flash(0n, 0n, ethers.ZeroAddress, 789n, 1234n))
            ...
            .to.emit(token1, "Transfer")
            .withArgs(wallet.address, await pool.getAddress(), 1234n);
      ...
      describe("fee on", () => {
        ...
        it("increases the fee growth by the expected amount", async () => {
          ...
          const [token0ProtocolFees, token1ProtocolFees] = await pool.protocolFees();
  ...
  describe("#increaseObservationCardinalityNext", () => {
    it("can only be called after initialize", async () => {
      await expect(pool.increaseObservationCardinalityNext(2)).to.be.revertedWithCustomError(
        pool,
        "LOK",
      );
    });
    ...
    describe("after initialization", () => {
      ...
      describe("failure cases", () => {
        it("fails if tickLower greater than tickUpper", async () => {
          ...
          await expect(mint(wallet.address, 0n, 1, 0, 1n)).to.be.reverted;
        });
        it("fails if tickLower less than min tick", async () => {
          // should be TLM but...hardhat
          await expect(mint(wallet.address, 0n, -887273, 0, 1n)).to.be.reverted;
        });
        it("fails if tickUpper greater than max tick", async () => {
          // should be TUM but...hardhat
          await expect(mint(wallet.address, 0n, 0, 887273, 1n)).to.be.reverted;
        });
        ...
        it("fails if total amount at tick exceeds the max", async () => {
          // these should fail with 'LO' but hardhat is bugged
          await mint(wallet.address, 0n, minTick + tickSpacing, maxTick - tickSpacing, 1000n);
        ...
        it("fails if amount is 0", async () => {
          await expect(mint(wallet.address, 0n, minTick + tickSpacing, maxTick - tickSpacing, 0n))
            .to.be.reverted;
        });
      });
      ...
      describe("success cases", () => {
        it("initial balances", async () => {
          expect(await token0.balanceOf(pool.getAddress())).to.eq(9996);
          expect(await token1.balanceOf(pool.getAddress())).to.eq(1000);
        });
        ...
        describe("above current price", () => {
          it("transfers token0 only", async () => {
            await expect(mint(wallet.address, 0n, -22980, 0, 10000n))
              .to.emit(token0, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 21549)
              .to.not.emit(token1, "Transfer");
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996 + 21549);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000);
          });

          it("max tick with max leverage", async () => {
            await mint(wallet.address, 0n, maxTick - tickSpacing, maxTick, 2n ** 102n);
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996 + 828011525);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000);
          });

          it("works for max tick", async () => {
            await expect(mint(wallet.address, 0n, -22980, maxTick, 10000n))
              .to.emit(token0, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 31549);
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996 + 31549);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000);
          });
          ...
          it("does not write an observation", async () => {
            checkObservationEquals(await pool.observations(0), {
              tickCumulative: -23028n * BigInt(TEST_POOL_START_TIME - TEST_POOL_START_PERIOD_TIME),
              blockTimestamp: BigInt(TEST_POOL_START_TIME),
              initialized: true,
              secondsPerLiquidityCumulativeX128: SECONDS_PER_LIQUIDITY_INIT,
            });
        ...
        describe("including current price", () => {
          it("price within range: transfers current price of both tokens", async () => {
            await expect(
              mint(wallet.address, 0n, minTick + tickSpacing, maxTick - tickSpacing, 100n),
            )
              .to.emit(token0, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 317)
              .to.emit(token1, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 32);
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996 + 317);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000 + 32);
          });

          it("initializes lower tick", async () => {
            await mint(wallet.address, 0n, minTick + tickSpacing, maxTick - tickSpacing, 100n);
          ...
          it("works for min/max tick", async () => {
            await expect(mint(wallet.address, 0n, minTick, maxTick, 10000n))
              .to.emit(token0, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 31623)
              .to.emit(token1, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 3163);
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996 + 31623);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000 + 3163);
          });

          it("removing works", async () => {
            await mint(wallet.address, 0n, minTick + tickSpacing, maxTick - tickSpacing, 100n);
            await pool.burn(0n, minTick + tickSpacing, maxTick - tickSpacing, 100);
            const { amount0, amount1 } = await pool.collect.staticCall(
              wallet.address,
              0n,
              minTick + tickSpacing,
          ...
          it("writes an observation", async () => {
            ...
            checkObservationEquals(await pool.observations(0), {
              tickCumulative:
                -23028n * BigInt(TEST_POOL_START_TIME - TEST_POOL_START_PERIOD_TIME) + -23028n,
              blockTimestamp: BigInt(TEST_POOL_START_TIME + 1),
              initialized: true,
              secondsPerLiquidityCumulativeX128:
                BigInt(SECONDS_PER_LIQUIDITY_INIT) + 107650226801941937191829992860413859n,
            });
          });
        });
        ...
        describe("below current price", () => {
          it("transfers token1 only", async () => {
            await expect(mint(wallet.address, 0n, -46080, -23040, 10000n))
              .to.emit(token1, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 2162)
              .to.not.emit(token0, "Transfer");
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000 + 2162);
          });

          it("min tick with max leverage", async () => {
            await mint(wallet.address, 0n, minTick, minTick + tickSpacing, 2n ** 102n);
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000 + 828011520);
          });

          it("works for min tick", async () => {
            await expect(mint(wallet.address, 0n, minTick, -23040, 10000n))
              .to.emit(token1, "Transfer")
              .withArgs(wallet.address, await pool.getAddress(), 3161);
            expect(await token0.balanceOf(pool.getAddress())).to.eq(9996);
            expect(await token1.balanceOf(pool.getAddress())).to.eq(1000 + 3161);
          });
      ...
      it("poke is not allowed on uninitialized position", async () => {
        ...
        await expect(pool.burn(0n, minTick + tickSpacing, maxTick - tickSpacing, 0)).to.be.reverted;

        await mint(wallet.address, 0n, minTick + tickSpacing, maxTick - tickSpacing, 1n);
        let {
          liquidity,
        ...
        await pool.burn(0n, minTick + tickSpacing, maxTick - tickSpacing, 1);
        ({
          liquidity,
          feeGrowthInside0LastX128,
          feeGrowthInside1LastX128,
  ...
  describe("#burn", () => {
    ...
    it("does not clear the position fee growth snapshot if no more liquidity", async () => {
      ...
      await mint(other.address, 0n, minTick, maxTick, expandTo18Decimals(1n));
      await swapExact0For1(expandTo18Decimals(1n), wallet.address);
      await swapExact1For0(expandTo18Decimals(1n), wallet.address);
      await pool.connect(other).burn(0n, minTick, maxTick, expandTo18Decimals(1n));
      const {
        liquidity,
        tokensOwed0,
        tokensOwed1,
    ...
    it("clears only the lower tick if upper is still used", async () => {
      ...
      await pool.advanceTime(10);
      await mint(wallet.address, 0n, tickLower, tickUpper, 1n);
      await mint(wallet.address, 0n, tickLower + tickSpacing, tickUpper, 1n);
      await swapExact0For1(expandTo18Decimals(1n), wallet.address);
      await pool.burn(0n, tickLower, tickUpper, 1);
      await checkTickIsClear(tickLower);
    ...
    it("clears only the upper tick if lower is still used", async () => {
      ...
      await pool.advanceTime(10);
      await mint(wallet.address, 0n, tickLower, tickUpper, 1n);
      await mint(wallet.address, 0n, tickLower, tickUpper - tickSpacing, 1n);
      await swapExact0For1(expandTo18Decimals(1n), wallet.address);
      await pool.burn(0n, tickLower, tickUpper, 1);
  ...
  describe("miscellaneous mint tests", () => {
    ...
    it("mint to the right of the current price", async () => {
      ...
      await mint(wallet.address, 0n, lowerTick, upperTick, liquidityDelta);
    ...
    it("cannot remove more than the entire position", async () => {
      const lowerTick = -tickSpacing;
      const upperTick = tickSpacing;
      await mint(wallet.address, 0n, lowerTick, upperTick, expandTo18Decimals(1000n));
      // should be 'LS', hardhat is bugged
      // no it's not, skill issue
      await expect(pool.burn(0n, lowerTick, upperTick, expandTo18Decimals(1001n))).to.be.reverted; //revertedWithCustomError(pool, "LS");
    });
    ...
    it("collect fees within the current price after swap", async () => {
      ...
      const token0BalanceBeforePool = await token0.balanceOf(pool.getAddress());
      const token1BalanceBeforePool = await token1.balanceOf(pool.getAddress());
      ...
      await pool.collect(wallet.address, 0n, lowerTick, upperTick, MaxUint128, MaxUint128);
      ...
      const token0BalanceAfterPool = await token0.balanceOf(pool.getAddress());
      const token1BalanceAfterPool = await token1.balanceOf(pool.getAddress());
    ...
    describe("k (implicit)", () => {
      ...
      describe("post initialized", () => {
        ...
        it("returns in supply in range", async () => {
          await mint(wallet.address, 0n, -tickSpacing, tickSpacing, expandTo18Decimals(3n));
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(5n));
        });
        it("excludes supply at tick above current tick", async () => {
          await mint(wallet.address, 0n, tickSpacing, tickSpacing * 2, expandTo18Decimals(3n));
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(2n));
        });
        it("excludes supply at tick below current tick", async () => {
          await mint(wallet.address, 0n, -tickSpacing * 2, -tickSpacing, expandTo18Decimals(3n));
          expect(await pool.liquidity()).to.eq(expandTo18Decimals(2n));
        });
  ...
  describe("limit orders", () => {
    ...
    it("limit selling 0 for 1 at tick 0 thru 1", async () => {
      await expect(mint(wallet.address, 0n, 0, 120, expandTo18Decimals(1n)))
        .to.emit(token0, "Transfer")
      ...
      await expect(pool.burn(0n, 0, 120, expandTo18Decimals(1n)))
        ...
        .to.not.emit(token0, "Transfer")
        .to.not.emit(token1, "Transfer");
      await expect(pool.collect(wallet.address, 0n, 0, 120, MaxUint128, MaxUint128))
        .to.emit(token1, "Transfer")
    ...
    it("limit selling 1 for 0 at tick 0 thru -1", async () => {
      await expect(mint(wallet.address, 0n, -120, 0, expandTo18Decimals(1n)))
        .to.emit(token1, "Transfer")
        .withArgs(wallet.address, await pool.getAddress(), "5981737760509663");
      // somebody takes the limit order
      ...
      await expect(pool.burn(0n, -120, 0, expandTo18Decimals(1n)))
        ...
        .to.not.emit(token0, "Transfer")
        .to.not.emit(token1, "Transfer");
      await expect(pool.collect(wallet.address, 0n, -120, 0, MaxUint128, MaxUint128))
        .to.emit(token0, "Transfer")
        .withArgs(await pool.getAddress(), wallet.address, 6017734268818165n + 18107525382602n); // roughly 0.3% despite other liquidity
    ...
    describe("fee is on", () => {
      ...
      it("limit selling 0 for 1 at tick 0 thru 1", async () => {
        ...
        await expect(pool.collect(wallet.address, 0n, 0, 120, MaxUint128, MaxUint128))
          .to.emit(token1, "Transfer")
          .withArgs(await pool.getAddress(), wallet.address, 6017734268818165n + 3621505076520n) // roughly 0.25% despite other liquidity
          .to.not.emit(token0, "Transfer");
        expect((await pool.slot0()).tick).to.be.gte(120);
      });
      it("limit selling 1 for 0 at tick 0 thru -1", async () => {
  ...
  describe("#collect", () => {
    beforeEach(async () => {
      pool = await createPool(TICK_SPACINGS[FeeAmount.LOW], encodePriceSqrt(1n, 1n).toString());
    });

    it("works with multiple LPs", async () => {
      await mint(wallet.address, 0n, minTick, maxTick, expandTo18Decimals(1n));
      await mint(
        wallet.address,
        0n,
        minTick + tickSpacing,
      ...
      await pool.burn(0n, minTick + tickSpacing, maxTick - tickSpacing, 0);
    ...
    describe("works across overflow boundaries", () => {
      beforeEach(async () => {
        ...
        await mint(wallet.address, 0n, minTick, maxTick, expandTo18Decimals(10n));
      });
  ...
  describe("#feeProtocol", () => {
    ...
    describe("#collectProtocol", () => {
      it("returns 0 if no fees", async () => {
        ...
        const { amount0, amount1 } = await pool.collectProtocol.staticCall(
          wallet.address,
          MaxUint128,
          MaxUint128,
        );
      ...
      it("can collect fees", async () => {
        ...
        await expect(pool.collectProtocol(other.address, MaxUint128, MaxUint128))
          .to.emit(token0, "Transfer")
          .withArgs(await pool.getAddress(), other.address, "399999999999999");
      });
    });
    ...
    it("fees collected by lp after two swaps with intermediate withdrawal", async () => {
      ...
      // collect the fees
      await pool.collect(wallet.address, 0n, minTick, maxTick, MaxUint128, MaxUint128);
      ...
      await expect(pool.collect(wallet.address, 0n, minTick, maxTick, MaxUint128, MaxUint128))
        .to.emit(token0, "Transfer")
        .withArgs(await pool.getAddress(), wallet.address, "99999999999999");
      protocolFees = await pool.protocolFees();
      expect(protocolFees[0]).to.eq("800000000000000");
      expect(protocolFees[1]).to.eq(0);
    });
    ...
    describe("tickSpacing = 12", () => {
      ...
      describe("post initialize", () => {
        ...
        it("mint can only be called for multiples of 12", async () => {
          await expect(mint(wallet.address, 0n, -6, 0, 1n)).to.be.reverted;
          await expect(mint(wallet.address, 0n, 0, 6, 1n)).to.be.reverted;
        });
        ...
        it("swapping across gaps works in 1 for 0 direction", async () => {
          ...
          await mint(wallet.address, 0n, 120000, 121200, liquidityAmount);
          await swapExact1For0(expandTo18Decimals(1n), wallet.address);
        ...
        it("swapping across gaps works in 0 for 1 direction", async () => {
          const liquidityAmount = expandTo18Decimals(1n) / 4n;
          await mint(wallet.address, 0n, -121200, -120000, liquidityAmount);
          await swapExact0For1(expandTo18Decimals(1n), wallet.address);
          await expect(pool.burn(0n, -121200, -120000, liquidityAmount))
            .to.emit(pool, "Burn")
            .withArgs(
              wallet.address,
              -121200,
  ...
  describe("#increaseObservationCardinalityNext", () => {
    ...
    describe("after initialization", () => {
      beforeEach("initialize the pool", () => pool.initialize(encodePriceSqrt(1n, 1n).toString()));
      it("oracle starting state after initialization", async () => {
        ...
        expect(secondsPerLiquidityCumulativeX128).to.eq(SECONDS_PER_LIQUIDITY_PERIOD_INIT);
        expect(tickCumulative).to.eq(0);
  ...
  describe("#setFeeProtocol", () => {
    ...
    it("emits an event when turned on", async () => {
      await factory.setFeeProtocol(7);
      await expect(pool.setFeeProtocol()).to.be.emit(pool, "SetFeeProtocol").withArgs(0, 7);
    });
    it("emits an event when turned off", async () => {
      ...
      await expect(pool.setFeeProtocol()).to.be.emit(pool, "SetFeeProtocol").withArgs(7, 0);
    });
    it("emits an event when changed", async () => {
      await factory.setFeeProtocol(4);
      await pool.setFeeProtocol();
      await factory.setFeeProtocol(6);
      await expect(pool.setFeeProtocol()).to.be.emit(pool, "SetFeeProtocol").withArgs(4, 6);
    });
    it("emits an event when unchanged", async () => {
      await factory.setFeeProtocol(5);
      await pool.setFeeProtocol();
      await factory.setFeeProtocol(5);
      await expect(pool.setFeeProtocol()).to.be.emit(pool, "SetFeeProtocol").withArgs(5, 5);
    });
  });
  ...
  describe("#lock", () => {
    ...
    it("cannot reenter from swap callback", async () => {
      ...
      await expect(reentrant.swapToReenter(await pool.getAddress())).to.be.revertedWith(
        "Unable to reenter",
      );
    });
  });
  ...
  describe("#snapshotCumulativesInside", () => {
    ...
    it("does not account for time increase above range", async () => {
      await pool.advanceTime(5);
      await swapToHigherPrice(BigInt(encodePriceSqrt(2n, 1n).toString()), wallet.address);
    ...
    it("does not account for time increase below range", async () => {
      await pool.advanceTime(5);
      await swapToLowerPrice(BigInt(encodePriceSqrt(1n, 2n).toString()), wallet.address);
      await pool.advanceTime(7);
      const { secondsPerLiquidityInsideX128, tickCumulativeInside, secondsInside } =
        await pool.snapshotCumulativesInside(tickLower, tickUpper);
      expect(secondsPerLiquidityInsideX128).to.eq((5n << 128n) / 10n);
    ...
    it("time increase below range is not counted", async () => {
      await swapToLowerPrice(BigInt(encodePriceSqrt(1n, 2n).toString()), wallet.address);
      await pool.advanceTime(5);
      await swapToHigherPrice(BigInt(encodePriceSqrt(1n, 1n).toString()), wallet.address);
      await pool.advanceTime(7);
      const { secondsPerLiquidityInsideX128, tickCumulativeInside, secondsInside } =
        await pool.snapshotCumulativesInside(tickLower, tickUpper);
    ...
    it("time increase above range is not counted", async () => {
      await swapToHigherPrice(BigInt(encodePriceSqrt(2n, 1n).toString()), wallet.address);
      await pool.advanceTime(5);
      await swapToLowerPrice(BigInt(encodePriceSqrt(1n, 1n).toString()), wallet.address);
      await pool.advanceTime(7);
      const { secondsPerLiquidityInsideX128, tickCumulativeInside, secondsInside } =
        await pool.snapshotCumulativesInside(tickLower, tickUpper);
      expect(secondsPerLiquidityInsideX128).to.eq((7n << 128n) / 10n);
    ...
    it("positions minted after time spent", async () => {
      await pool.advanceTime(5);
      await mint(wallet.address, 0n, tickUpper, getMaxTick(tickSpacing), 15n);
      await swapToHigherPrice(BigInt(encodePriceSqrt(2n, 1n).toString()), wallet.address);
    ...
    it("overlapping liquidity is aggregated", async () => {
      await mint(wallet.address, 0n, tickLower, getMaxTick(tickSpacing), 15n);
      await pool.advanceTime(5);
      await swapToHigherPrice(BigInt(encodePriceSqrt(2n, 1n).toString()), wallet.address);
      await pool.advanceTime(8);
      const { secondsPerLiquidityInsideX128, tickCumulativeInside, secondsInside } =
    ...
    it("relative behavior of snapshots", async () => {
      await pool.advanceTime(5);
      await mint(wallet.address, 0n, getMinTick(tickSpacing), tickLower, 15n);
      console.log("here");
      const {
      ...
      expect(tickCumulativeInside - tickCumulativeInsideStart, "tickCumulativeInside").to.eq(
        -20796,
      );
  ...
  describe("fees overflow scenarios", async () => {
    ...
    it("two positions at the same snapshot", async () => {
      ...
      ({ amount0 } = await pool
        .connect(other)
        .collect.staticCall(other.address, 0n, minTick, maxTick, MaxUint128, MaxUint128));
  ...
  describe("swap underpayment tests", () => {
    ...
    it("underpay zero for one and exact in", async () => {
      await expect(
        underpay.swap(pool.getAddress(), wallet.address, true, MIN_SQRT_RATIO + 1n, 1000, 1, 0),
      ).to.be.revertedWithCustomError(pool, "IIA");
    });
    it("pay in the wrong token zero for one and exact in", async () => {
      await expect(
        underpay.swap(pool.getAddress(), wallet.address, true, MIN_SQRT_RATIO + 1n, 1000, 0, 2000),
      ).to.be.revertedWithCustomError(pool, "IIA");
    });
    it("overpay zero for one and exact in", async () => {
      await expect(
        underpay.swap(pool.getAddress(), wallet.address, true, MIN_SQRT_RATIO + 1n, 1000, 2000, 0),
    ...
    it("underpay one for zero and exact in", async () => {
      await expect(
        underpay.swap(pool.getAddress(), wallet.address, false, MAX_SQRT_RATIO - 1n, 1000, 0, 1),
      ).to.be.revertedWithCustomError(pool, "IIA");
    });
    it("pay in the wrong token one for zero and exact in", async () => {
      await expect(
        underpay.swap(pool.getAddress(), wallet.address, false, MAX_SQRT_RATIO - 1n, 1000, 2000, 0),
      ).to.be.revertedWithCustomError(pool, "IIA");
    });
    it("overpay one for zero and exact in", async () => {
      await expect(
        underpay.swap(pool.getAddress(), wallet.address, false, MAX_SQRT_RATIO - 1n, 1000, 0, 2000),
      ).to.not.be.revertedWithCustomError(pool, "IIA");
    });
