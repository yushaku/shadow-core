...
import { MockTimeUniswapV3Pool, TickMathTest, UniswapV3PoolSwapTest } from "../../typechain-types";
...
function applySqrtRatioBipsHundredthsDelta(sqrtRatio: bigint, bipsHundredths: number): bigint {
  return BigInt(
    new Decimal(
      ((sqrtRatio * sqrtRatio * (10n ** 6n + BigInt(bipsHundredths))) / 10n ** 6n).toString(),
    )
      .sqrt()
      .floor()
      .toString(),
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          async function arbTestFixture() {
            ...
            await mint(wallet.address, 0n, minTick, maxTick, passiveLiquidity);
          ...
          async function simulateSwap(
            ...
            const executionPrice = zeroForOne
              ? BigInt(encodePriceSqrt(amount1Delta, amount0Delta * -1n).toString())
              : BigInt(encodePriceSqrt(amount1Delta * -1n, amount0Delta).toString());
          ...
          for (const { zeroForOne, assumedTruePriceAfterSwap, inputAmount, description } of [
            {
              ...
              assumedTruePriceAfterSwap: BigInt(encodePriceSqrt(98n, 100n).toString()),
            },
            {
              description:
              ...
              zeroForOne: true,
              inputAmount: expandTo18Decimals(10n),
              assumedTruePriceAfterSwap: BigInt(encodePriceSqrt(101n, 100n).toString()),
            },
          ]) {
            describe(description, () => {
              function valueToken1(arbBalance0: bigint, arbBalance1: bigint) {
                return (
                  (assumedTruePriceAfterSwap * assumedTruePriceAfterSwap * arbBalance0) /
                    2n ** 192n +
                  arbBalance1
                );
              }
              ...
              it("sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity", async () => {
                ...
                // sub the mint costs
                const { amount0: amount0Mint, amount1: amount1Mint } = events[0].args;
                ...
                await pool.burn(0, tickLower, tickUpper, getMaxLiquidityPerTick(tickSpacing));
