...
import { MockTimeClPool, TestERC20, TestClPoolCallee } from "../../typechain-types";
...
describe.skip("ClPool tax token swap tests", () => {
  ...
  for (const poolCase of TEST_POOLS) {
    describe(poolCase.description, () => {
      async function poolCaseFixture() {
        const {
          createPool,
          taxToken0: token0, // using fee-on-transfer tokens for this test
          taxToken1: token1,
        ...
        for (const position of poolCase.positions) {
          ...
          const extra0 = amount0.div(100).mul(100).div(99).add(1);
          const extra1 = amount1.div(100).mul(100).div(99).add(1);

          console.log("extra0", extra0);
          console.log("extra1", extra1);

          console.log(extra0.sub(extra0.div(100)).add(amount0.sub(amount0.div(100))));
      ...
      afterEach("check can burn positions", async () => {
        for (const { liquidity, tickUpper, tickLower } of poolCase.positions) {
          await pool["burn(int24,int24,uint128)"](tickLower, tickUpper, liquidity);
