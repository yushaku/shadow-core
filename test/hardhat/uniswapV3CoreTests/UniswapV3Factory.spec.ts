...
import { UniswapV3Factory, UniswapV3PoolDeployer } from "../../typechain-types";
...
describe("UniswapV3Factory", () => {
  ...
  it("pool bytecode size", async () => {
    ...
    const poolAddress = getCreate2Address(
      await factory.uniswapV3PoolDeployer(),
      TEST_ADDRESSES,
      60,
      poolBytecode,
    );
  ...
  it("initial enabled tickSpacings", async () => {
    ...
    expect(await factory.tickSpacingInitialFee(60)).to.eq(3000);
    expect(await factory.tickSpacingInitialFee(200)).to.eq(10000);
  });
  ...
  async function createAndCheckPool(
    ...
    await expect(
      factory.createPool(tokens[0], tokens[1], tickSpacing, encodePriceSqrt(1n, 1n).toString()),
    ).to.be.reverted;
    await expect(
      factory.createPool(tokens[1], tokens[0], tickSpacing, encodePriceSqrt(1n, 1n).toString()),
    ).to.be.reverted;
    expect(await factory.getPool(tokens[0], tokens[1], tickSpacing), "getPool in order").to.eq(
      create2Address,
    );
    expect(await factory.getPool(tokens[1], tokens[0], tickSpacing), "getPool in reverse").to.eq(
      create2Address,
    );

    const pool = await ethers.getContractAt("UniswapV3Pool", create2Address);
  ...
  describe("#enableFeeAmount", () => {
    it("fails if fee is too great", async () => {
      await expect(factory.enableTickSpacing(300, 1000000)).to.be.reverted;
    });
    ...
    it("fails if already initialized", async () => {
      await factory.enableTickSpacing(5, 99);
      await expect(factory.enableTickSpacing(5, 99)).to.be.reverted;
    });
    it("sets the fee amount in the mapping", async () => {
      await factory.enableTickSpacing(5, 99);
      expect(await factory.tickSpacingInitialFee(5)).to.eq(99);
    });
    it("emits an event", async () => {
      await expect(factory.enableTickSpacing(5, 99))
