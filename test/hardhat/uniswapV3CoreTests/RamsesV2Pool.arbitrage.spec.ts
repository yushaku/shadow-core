...
import { MockTimeClPool, TickMathTest, ClPoolSwapTest } from "../../typechain-types";
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          async function arbTestFixture() {
            ...
            const pool = await fix.createPool(feeAmount, startingPrice);
            ...
            if (feeProtocol != 0)
              await pool["setFeeProtocol(uint8,uint8)"](feeProtocol, feeProtocol);
            ...
            describe(description, () => {
              ...
              it("sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity", async () => {
                ...
                const { amount0: amount0CollectAndBurn, amount1: amount1CollectAndBurn } =
                  await pool.callStatic["collect(address,int24,int24,uint128,uint128)"](
                    arbitrageur.address,
                    tickLower,
                    tickUpper,
                    MaxUint128,
