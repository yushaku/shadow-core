...
describe("ClPool", () => {
    ...
    describe("#initialize", () => {
        it("fails if already initialized", async () => {
            ...
            await expect(pool["initialize(uint160)"](encodePriceSqrt(1, 1))).to
                .be.reverted;
        });
        it("fails if starting price is too low", async () => {
            await expect(pool["initialize(uint160)"](1)).to.be.revertedWith(
                "R"
            );
            await expect(
        ...
        describe("after initialization", () => {
            ...
            describe("failure cases", () => {
                ...
                it("fails if tickLower less than min tick", async () => {
                    ...
                    await expect(mint(wallet.address, -887273, 0, 1)).to.be
                        .reverted;
                });
                it("fails if tickUpper greater than max tick", async () => {
                    // should be TUM but...hardhat
                    await expect(mint(wallet.address, 0, 887273, 1)).to.be
                        .reverted;
                });
            ...
            describe("success cases", () => {
                ...
                describe("above current price", () => {
                    ...
                    it("works for max tick", async () => {
                        await expect(
                            mint(wallet.address, -22980, maxTick, 10000)
                        )
                            .to.emit(token0, "Transfer")
                ...
                describe("including current price", () => {
                    ...
                    it("works for min/max tick", async () => {
                        await expect(
                            mint(wallet.address, minTick, maxTick, 10000)
                        )
                            .to.emit(token0, "Transfer")
                    ...
                    it("writes an observation", async () => {
                        ...
                        checkObservationEquals(await pool.observations(0), {
                            ...
                            initialized: true,
                            secondsPerLiquidityCumulativeX128: BigNumber.from(
                                SECONDS_PER_LIQUIDITY_INIT
                            ).add("107650226801941937191829992860413859"),
                        });
                    });
                });
                ...
                describe("below current price", () => {
                    it("transfers token1 only", async () => {
                        await expect(
                            mint(wallet.address, -46080, -23040, 10000)
                        )
                            .to.emit(token1, "Transfer")
                    ...
                    it("works for min tick", async () => {
                        await expect(
                            mint(wallet.address, minTick, -23040, 10000)
                        )
                            .to.emit(token1, "Transfer")
    ...
    describe("limit orders", () => {
        ...
        it("limit selling 0 for 1 at tick 0 thru 1", async () => {
            ...
            await expect(
                pool["burn(int24,int24,uint128)"](0, 120, expandTo18Decimals(1))
            )
                .to.emit(pool, "Burn")
                .withArgs(
                    wallet.address,
        ...
        describe("fee is on", () => {
            ...
            it("limit selling 0 for 1 at tick 0 thru 1", async () => {
                await expect(
                    mint(wallet.address, 0, 120, expandTo18Decimals(1))
                )
                    .to.emit(token0, "Transfer")
                    .withArgs(wallet.address, pool.address, "5981737760509663");
                // somebody takes the limit order
            ...
            it("limit selling 1 for 0 at tick 0 thru -1", async () => {
                await expect(
                    mint(wallet.address, -120, 0, expandTo18Decimals(1))
                )
                    .to.emit(token1, "Transfer")
                    .withArgs(wallet.address, pool.address, "5981737760509663");
                // somebody takes the limit order
    ...
    describe("#feeProtocol", () => {
        ...
        it("cannot be changed out of bounds", async () => {
            await expect(pool["setFeeProtocol(uint8,uint8)"](3, 3)).to.be
                .reverted;
            await expect(pool["setFeeProtocol(uint8,uint8)"](11, 11)).to.be
                .reverted;
        });

        it("cannot be changed by addresses that are not owner", async () => {
            await expect(
                pool.connect(other)["setFeeProtocol(uint8,uint8)"](6, 6)
            ).to.be.reverted;
        });
        ...
        describe("#collectProtocol", () => {
            it("returns 0 if no fees", async () => {
                ...
                const { amount0, amount1 } =
                    await pool.callStatic.collectProtocol(
                        wallet.address,
                        MaxUint128,
                        MaxUint128
                    );
        ...
        describe("after liquidity added", () => {
            ...
            describe("fee off", () => {
                ...
                it("fails if flash amount is greater than token balance", async () => {
                    await expect(
                        flash(balance0.add(1), balance1, other.address)
                    ).to.be.reverted;
                    await expect(
                        flash(balance0, balance1.add(1), other.address)
                    ).to.be.reverted;
                });
                ...
                it("fails if original balance not returned in either token", async () => {
                    await expect(flash(1000, 0, other.address, 999, 0)).to.be
                        .reverted;
                    await expect(flash(0, 1000, other.address, 0, 999)).to.be
                        .reverted;
                });
                it("fails if underpays either token", async () => {
                    await expect(flash(1000, 0, other.address, 1002, 0)).to.be
                        .reverted;
                    await expect(flash(0, 1000, other.address, 0, 1002)).to.be
                        .reverted;
                });
            ...
            describe("fee on", () => {
                ...
                it("allows donating token0", async () => {
                    ...
                    const { token0: token0ProtocolFees } =
                        await pool.protocolFees();
                ...
                it("allows donating token1", async () => {
                    ...
                    const { token1: token1ProtocolFees } =
                        await pool.protocolFees();
    ...
    describe("#setFeeProtocol", () => {
        ...
        it("can only be called by factory owner", async () => {
            await expect(
                pool.connect(other)["setFeeProtocol(uint8,uint8)"](5, 5)
            ).to.be.reverted;
        });
        it("fails if fee is lt 4 or gt 10", async () => {
            await expect(pool["setFeeProtocol(uint8,uint8)"](3, 3)).to.be
                .reverted;
            await expect(pool["setFeeProtocol(uint8,uint8)"](6, 3)).to.be
                .reverted;
            await expect(pool["setFeeProtocol(uint8,uint8)"](3, 6)).to.be
                .reverted;
            await expect(pool["setFeeProtocol(uint8,uint8)"](11, 11)).to.be
                .reverted;
            await expect(pool["setFeeProtocol(uint8,uint8)"](6, 11)).to.be
                .reverted;
            await expect(pool["setFeeProtocol(uint8,uint8)"](11, 6)).to.be
                .reverted;
        });
    ...
    describe("#lock", () => {
        ...
        it("cannot reenter from swap callback", async () => {
            ...
            await expect(
                reentrant.swapToReenter(pool.address)
    ...
    describe("#snapshotCumulativesInside", () => {
        ...
        it("relative behavior of snapshots", async () => {
            ...
            const expectedDiffSecondsPerLiquidity = BigNumber.from(3)
                ...
                .div(15);
            expect(
                secondsPerLiquidityInsideX128.sub(
                    secondsPerLiquidityInsideX128Start
                )
    ...
    describe("fees overflow scenarios", async () => {
        ...
        it("two positions at the same snapshot", async () => {
            ...
            await pool
                .connect(other)
                ["burn(int24,int24,uint128)"](minTick, maxTick, 0);
            let { amount0 } = await pool.callStatic[
                "collect(address,int24,int24,uint128,uint128)"
            ](wallet.address, minTick, maxTick, MaxUint128, MaxUint128);
