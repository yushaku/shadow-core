...
describe("ClPool", () => {
  ...
  let factory: MockTimeClPoolFactory;
  ...
  beforeEach("deploy fixture", async () => {
    ...
    createPool = async (_feeAmount, _startingPrice, _tickSpacing) => {
      const pool = await oldCreatePool(_feeAmount, _startingPrice, _tickSpacing);
      ({
        swapToLowerPrice,
        swapToHigherPrice,
        swapExact0For1,
    ...
    pool = await createPool(FeeAmount.MEDIUM, BigNumber.from(0), TICK_SPACINGS[FeeAmount.MEDIUM]);
  ...
  it("code size within limit", async () => {
    ...
    hre.config.contractSizer.only = ["ClPool"];
  ...
  describe("#initialize", () => {
    it("fails if already initialized", async () => {
      ...
      await expect(pool["initialize(uint160)"](encodePriceSqrt(1, 1))).to.be.reverted;
    });
    it("fails if starting price is too low", async () => {
      await expect(pool["initialize(uint160)"](1)).to.be.revertedWith("R");
      await expect(pool["initialize(uint160)"](MIN_SQRT_RATIO.sub(1))).to.be.revertedWith("R");
    });
    it("fails if starting price is too high", async () => {
      await expect(pool["initialize(uint160)"](MAX_SQRT_RATIO)).to.be.revertedWith("R");
      await expect(
    ...
    it("initializes the first observations slot", async () => {
      ...
      checkObservationEquals(await pool.observations(0), {
        ...
        initialized: true,
        blockTimestamp: TEST_POOL_START_PERIOD_TIME,
        tickCumulative: 0,
      });
    });
  ...
  describe("#mint", () => {
    ...
    describe("after initialization", () => {
      ...
      describe("success cases", () => {
        ...
        describe("above current price", () => {
          ...
          it("removing works", async () => {
            ...
            const { amount0, amount1 } = await pool.callStatic[
              "collect(address,int24,int24,uint128,uint128)"
            ](wallet.address, -240, 0, MaxUint128, MaxUint128);
          ...
          it("does not write an observation", async () => {
            checkObservationEquals(await pool.observations(0), {
              tickCumulative: BigNumber.from(-23028)
                .mul(TEST_POOL_START_TIME - TEST_POOL_START_PERIOD_TIME)
                .toString(),
        ...
        describe("including current price", () => {
          ...
          it("removing works", async () => {
            ...
            const { amount0, amount1 } = await pool.callStatic[
              "collect(address,int24,int24,uint128,uint128)"
            ](wallet.address, minTick + tickSpacing, maxTick - tickSpacing, MaxUint128, MaxUint128);
          ...
          it("writes an observation", async () => {
            ...
            checkObservationEquals(await pool.observations(0), {
              ...
              initialized: true,
              secondsPerLiquidityCumulativeX128: BigNumber.from(SECONDS_PER_LIQUIDITY_INIT).add(
        ...
        describe("below current price", () => {
          ...
          it("removing works", async () => {
            ...
            const { amount0, amount1 } = await pool.callStatic[
              "collect(address,int24,int24,uint128,uint128)"
            ](wallet.address, -46080, -46020, MaxUint128, MaxUint128);
      ...
      it("protocol fees accumulate as expected during swap", async () => {
        await pool["setFeeProtocol(uint8,uint8)"](6, 6);
        ...
        expect(token0ProtocolFees).to.eq("240000000000000");
        expect(token1ProtocolFees).to.eq("24000000000000");
      });
      ...
      it("poke is not allowed on uninitialized position", async () => {
        ...
        await expect(
          pool["burn(int24,int24,uint128)"](minTick + tickSpacing, maxTick - tickSpacing, 0),
        ).to.be.reverted;
  ...
  describe("#burn", () => {
    ...
    it("does not clear the position fee growth snapshot if no more liquidity", async () => {
      ...
      await pool
        .connect(other)
        ["burn(int24,int24,uint128)"](minTick, maxTick, expandTo18Decimals(1));
      const {
        liquidity,
        tokensOwed0,
        tokensOwed1,
  ...
  describe("miscellaneous mint tests", () => {
    beforeEach("initialize at zero tick", async () => {
      pool = await createPool(FeeAmount.LOW, BigNumber.from(0), TICK_SPACINGS[FeeAmount.LOW]);
    ...
    it("cannot remove more than the entire position", async () => {
      ...
      await expect(
        pool["burn(int24,int24,uint128)"](lowerTick, upperTick, expandTo18Decimals(1001)),
      ).to.be.reverted;
    });
    ...
    it("collect fees within the current price after swap", async () => {
      ...
      await pool["burn(int24,int24,uint128)"](lowerTick, upperTick, 0);
      const { amount0: fees0, amount1: fees1 } = await pool.callStatic[
        "collect(address,int24,int24,uint128,uint128)"
      ](wallet.address, lowerTick, upperTick, MaxUint128, MaxUint128);
  ...
  describe("limit orders", () => {
    ...
    describe("fee is on", () => {
      beforeEach(() => pool["setFeeProtocol(uint8,uint8)"](6, 6));
      it("limit selling 0 for 1 at tick 0 thru 1", async () => {
        await expect(mint(wallet.address, 0, 120, expandTo18Decimals(1)))
          .to.emit(token0, "Transfer")
          .withArgs(wallet.address, pool.address, "5981737760509663");
        ...
        await expect(
          ...
          .withArgs(
            pool.address,
            wallet.address,
            BigNumber.from("6017734268818165").add("3621505076520"),
  ...
  describe("#collect", () => {
    beforeEach(async () => {
      pool = await createPool(FeeAmount.LOW, encodePriceSqrt(1, 1), TICK_SPACINGS[FeeAmount.LOW]);
    });
    ...
    describe("works across overflow boundaries", () => {
      ...
      it("token0", async () => {
        ...
        const { amount0, amount1 } = await pool.callStatic[
          "collect(address,int24,int24,uint128,uint128)"
        ](wallet.address, minTick, maxTick, MaxUint128, MaxUint128);
  ...
  describe("#feeProtocol", () => {
    ...
    it("cannot be changed out of bounds", async () => {
      await expect(pool["setFeeProtocol(uint8,uint8)"](3, 3)).to.be.reverted;
      await expect(pool["setFeeProtocol(uint8,uint8)"](11, 11)).to.be.reverted;
    });

    it("cannot be changed by addresses that are not owner", async () => {
      await expect(pool.connect(other)["setFeeProtocol(uint8,uint8)"](6, 6)).to.be.reverted;
    });
    ...
    async function swapAndGetFeesOwed({
      ...
      if (poke) await pool["burn(int24,int24,uint128)"](minTick, maxTick, 0);
    ...
    describe("#collectProtocol", () => {
      ...
      it("can collect fees", async () => {
        ...
        await expect(pool.collectProtocol(other.address, MaxUint128, MaxUint128))
          .to.emit(token0, "Transfer")
          .withArgs(pool.address, other.address, "399999999999999");
      });

      it("fees collected can differ between token0 and token1", async () => {
        await pool["setFeeProtocol(uint8,uint8)"](8, 5);
        ...
        await expect(pool.collectProtocol(other.address, MaxUint128, MaxUint128))
          .to.emit(token0, "Transfer")
          // more token0 fees because it's 90% the swap fees
          .withArgs(pool.address, other.address, "449999999999999")
          .to.emit(token1, "Transfer")
          // less token1 fees because it's 75% the swap fees
          .withArgs(pool.address, other.address, "374999999999999");
      });
    });
    ...
    it("fees collected by lp after two swaps with intermediate withdrawal", async () => {
      ...
      let { token0: token0ProtocolFees, token1: token1ProtocolFees } = await pool.protocolFees();
      expect(token0ProtocolFees).to.eq("800000000000000");
      expect(token1ProtocolFees).to.eq(0);

      await pool["burn(int24,int24,uint128)"](minTick, maxTick, 0); // poke to update fees
      await expect(
        pool["collect(address,int24,int24,uint128,uint128)"](
          wallet.address,
          minTick,
        ...
        .to.emit(token0, "Transfer")
        .withArgs(pool.address, wallet.address, "99999999999999");
      ({ token0: token0ProtocolFees, token1: token1ProtocolFees } = await pool.protocolFees());
      expect(token0ProtocolFees).to.eq("800000000000000");
      expect(token1ProtocolFees).to.eq(0);
    });
  ...
  describe("#tickSpacing", () => {
    describe("tickSpacing = 12", () => {
      beforeEach("deploy pool", async () => {
        pool = await createPool(FeeAmount.MEDIUM, BigNumber.from(0), 12);
      });
  ...
  it("tick transition cannot run twice if zero for one swap ends at fractional price just below tick", async () => {
    pool = await createPool(FeeAmount.MEDIUM, BigNumber.from(0), 1);
  ...
  describe("#increaseObservationCardinalityNext", () => {
    ...
    describe("after initialization", () => {
      beforeEach("initialize the pool", () => pool["initialize(uint160)"](encodePriceSqrt(1, 1)));
  ...
  describe("#setFeeProtocol", () => {
    ...
    it("can only be called by factory owner", async () => {
      await expect(pool.connect(other)["setFeeProtocol(uint8,uint8)"](5, 5)).to.be.reverted;
    });
    it("fails if fee is lt 4 or gt 10", async () => {
      await expect(pool["setFeeProtocol(uint8,uint8)"](3, 3)).to.be.reverted;
      await expect(pool["setFeeProtocol(uint8,uint8)"](6, 3)).to.be.reverted;
      await expect(pool["setFeeProtocol(uint8,uint8)"](3, 6)).to.be.reverted;
      await expect(pool["setFeeProtocol(uint8,uint8)"](11, 11)).to.be.reverted;
      await expect(pool["setFeeProtocol(uint8,uint8)"](6, 11)).to.be.reverted;
      await expect(pool["setFeeProtocol(uint8,uint8)"](11, 6)).to.be.reverted;
    });
    it("succeeds for fee of 4", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](4, 4);
    });
    it("succeeds for fee of 10", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](10, 10);
    });
    it("sets protocol fee", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](7, 7);
      expect((await pool.slot0()).feeProtocol).to.eq(119);
    });
    it("can change protocol fee", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](7, 7);
      await pool["setFeeProtocol(uint8,uint8)"](5, 8);
      expect((await pool.slot0()).feeProtocol).to.eq(133);
    });
    it("can turn off protocol fee", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](4, 4);
      await pool["setFeeProtocol(uint8,uint8)"](0, 0);
      expect((await pool.slot0()).feeProtocol).to.eq(0);
    });
    it("emits an event when turned on", async () => {
      await expect(pool["setFeeProtocol(uint8,uint8)"](7, 7))
        .to.be.emit(pool, "SetFeeProtocol")
        .withArgs(0, 0, 7, 7);
    });
    it("emits an event when turned off", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](7, 5);
      await expect(pool["setFeeProtocol(uint8,uint8)"](0, 0))
        .to.be.emit(pool, "SetFeeProtocol")
        .withArgs(7, 5, 0, 0);
    });
    it("emits an event when changed", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](4, 10);
      await expect(pool["setFeeProtocol(uint8,uint8)"](6, 8))
        .to.be.emit(pool, "SetFeeProtocol")
        .withArgs(4, 10, 6, 8);
    });
    it("emits an event when unchanged", async () => {
      await pool["setFeeProtocol(uint8,uint8)"](5, 9);
      await expect(pool["setFeeProtocol(uint8,uint8)"](5, 9))
        .to.be.emit(pool, "SetFeeProtocol")
        .withArgs(5, 9, 5, 9);
    });
  });
  ...
  describe("#lock", () => {
    ...
    it("cannot reenter from swap callback", async () => {
      const reentrant = (await (
        await ethers.getContractFactory("TestClPoolReentrantCallee")
      ).deploy()) as TestClPoolReentrantCallee;
  ...
  describe("fees overflow scenarios", async () => {
    ...
    it("two positions at the same snapshot", async () => {
      ...
      ({ amount0 } = await pool
        ...
        .callStatic[
          "collect(address,int24,int24,uint128,uint128)"
        ](other.address, minTick, maxTick, MaxUint128, MaxUint128));
  ...
  describe("swap underpayment tests", () => {
    let underpay: TestClPoolSwapPay;
    beforeEach("deploy swap test", async () => {
      const underpayFactory = await ethers.getContractFactory("TestClPoolSwapPay");
      underpay = (await underpayFactory.deploy()) as TestClPoolSwapPay;
