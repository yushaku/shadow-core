...
describe("NonfungibleTokenPositionDescriptor", () => {
    ...
    describe("#tokenRatioPriority", () => {
        ...
        it("returns 200 for USDC", async () => {
            expect(
                await nftPositionDescriptor.tokenRatioPriority(USDC, 1)
            ).to.eq(300);
        });

        it("returns 100 for DAI", async () => {
            expect(
                await nftPositionDescriptor.tokenRatioPriority(DAI, 1)
            ).to.eq(100);
        });

        it("returns  150 for USDT", async () => {
            expect(
                await nftPositionDescriptor.tokenRatioPriority(USDT, 1)
            ).to.eq(200);
        });

        it("returns -200 for TBTC", async () => {
            expect(
                await nftPositionDescriptor.tokenRatioPriority(TBTC, 1)
            ).to.eq(-200);
        });

        it("returns -250 for WBTC", async () => {
            expect(
                await nftPositionDescriptor.tokenRatioPriority(WBTC, 1)
            ).to.eq(-300);
        });

        it("returns 0 for any non-ratioPriority token", async () => {
            expect(
                await nftPositionDescriptor.tokenRatioPriority(
                    tokens[0].address,
                    1
                )
            ).to.eq(0);
    ...
    describe("#flipRatio", () => {
        ...
        it("returns true if both tokens are numerators but token0 has a higher priority ordering", async () => {
            expect(await nftPositionDescriptor.flipRatio(USDC, DAI, 1)).to.eq(
                true
            );
        });
        ...
        it("returns true if token0 is a numerator and token1 is a denominator", async () => {
            expect(await nftPositionDescriptor.flipRatio(DAI, WBTC, 1)).to.eq(
                true
            );
        });

        it("returns false if token1 is a numerator and token0 is a denominator", async () => {
            expect(await nftPositionDescriptor.flipRatio(WBTC, DAI, 1)).to.eq(
                false
            );
        });
    });
    ...
    describe("#tokenURI", () => {
        ...
        it("can render a different label for native currencies", async () => {
            ...
            const nftDescriptorLibrary =
                await nftDescriptorLibraryFactory.deploy();
            ...
            expect(metadata.name).to.match(
                /(\sFUNNYMONEY\/TEST|TEST\/FUNNYMONEY)/
            );
            expect(metadata.description).to.match(
