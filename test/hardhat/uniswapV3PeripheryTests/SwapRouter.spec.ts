    ...
    describe("swaps", () => {
        ...
        describe("#exactInput", () => {
            async function exactInput(
                ...
                const outputIsWETH9 =
                    tokens[tokens.length - 1] === weth9.address;
            ...
            describe("single-pool", () => {
                it("0 -> 1", async () => {
                    ...
                    expect(traderAfter.token0).to.be.eq(
                        traderBefore.token0.sub(3)
                    );
                    expect(traderAfter.token1).to.be.eq(
                        traderBefore.token1.add(1)
                    );
                ...
                it("1 -> 0", async () => {
                    ...
                    expect(traderAfter.token0).to.be.eq(
                        traderBefore.token0.add(1)
                    );
                    expect(traderAfter.token1).to.be.eq(
                        traderBefore.token1.sub(3)
                    );
            ...
            describe("multi-pool", () => {
                it("0 -> 1 -> 2", async () => {
                    ...
                    expect(traderAfter.token0).to.be.eq(
                        traderBefore.token0.sub(5)
                    );
                    expect(traderAfter.token2).to.be.eq(
                        traderBefore.token2.add(1)
                    );
                });
            ...
            describe("ETH input", () => {
                describe("WETH9", () => {
                    ...
                    it("WETH9 -> 0", async () => {
                        ...
                        await expect(
                            exactInput([weth9.address, tokens[0].address])
                        )
                        ...
                        expect(poolAfter.weth9).to.be.eq(
                            poolBefore.weth9.add(3)
                        );
                        expect(poolAfter.token0).to.be.eq(
                            poolBefore.token0.sub(1)
                        );
                    });
            ...
            describe("ETH output", () => {
                describe("WETH9", () => {
                    ...
                    it("0 -> WETH9", async () => {
                        ...
                        await expect(
                            exactInput([tokens[0].address, weth9.address])
                        )
                        ...
                        expect(poolAfter.weth9).to.be.eq(
                            poolBefore.weth9.sub(1)
                        );
                        expect(poolAfter.token0).to.be.eq(
                            poolBefore.token0.add(3)
                        );
                    });
            ...
            describe("ETH input", () => {
                describe("WETH9", () => {
                    ...
                    it("WETH9 -> 0", async () => {
                        ...
                        await expect(
                            exactInputSingle(weth9.address, tokens[0].address)
                        )
                            .to.emit(weth9, "Deposit")
                            .withArgs(router.address, 3);
            ...
            describe("ETH output", () => {
                describe("WETH9", () => {
                    ...
                    it("0 -> WETH9", async () => {
                        ...
                        await expect(
                            exactInputSingle(tokens[0].address, weth9.address)
                        )
                            .to.emit(weth9, "Withdrawal")
                            .withArgs(router.address, 1);
        ...
        describe("#exactOutput", () => {
            async function exactOutput(
                ...
                const data = [
                    router.interface.encodeFunctionData("exactOutput", [
                        params,
                    ]),
                ];
                if (inputIsWETH9)
            ...
            describe("ETH input", () => {
                describe("WETH9", () => {
                    ...
                    it("WETH9 -> 0", async () => {
                        ...
                        await expect(
                            exactOutput([weth9.address, tokens[0].address])
                        )
                            .to.emit(weth9, "Deposit")
                            .withArgs(router.address, 3);
            ...
            describe("ETH output", () => {
                describe("WETH9", () => {
                    ...
                    it("0 -> WETH9", async () => {
                        ...
                        await expect(
                            exactOutput([tokens[0].address, weth9.address])
                        )
                            .to.emit(weth9, "Withdrawal")
                            .withArgs(router.address, 1);
        ...
        describe("*WithFee", () => {
            ...
            it("#unwrapWETH9WithFee", async () => {
                ...
                await router.connect(trader).multicall(data);
                const endBalance = await ethers.provider.getBalance(
                    feeRecipient
                );
