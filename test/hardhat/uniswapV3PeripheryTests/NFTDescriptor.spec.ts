...
describe("NFTDescriptor", () => {
    ...
    async function nftDescriptorFixture(): Promise<{
        tokens: [
            TestERC20Metadata,
            TestERC20Metadata,
            TestERC20Metadata,
    ...
    describe("#addressToString", () => {
        it("returns the correct string for a given address", async () => {
            ...
            expect(addressStr).to.eq(
                "0x1234abcdef1234abcdef1234abcdef1234abcdef"
            );
            addressStr = await nftDescriptor.addressToString(
                `0x${"1".repeat(40)}`
            );
        ...
        describe("when tokens have different decimal precision", () => {
            ...
            it("some fuzz", async () => {
                ...
                for (let i in inputs) {
                    ...
                    expect(
                        formatSqrtRatioX96(ratio, decimals0, decimals1)
                    ).to.eq(result);
                }
            }).timeout(300_000);
        });
    ...
    describe("#feeToPercentString", () => {
        ...
        it("returns the correct fee for 102000", async () => {
            expect(await nftDescriptor.feeToPercentString(102000)).to.eq(
                "10.2%"
            );
        });

        it("returns the correct fee for 10000000", async () => {
            expect(await nftDescriptor.feeToPercentString(1000000)).to.eq(
                "100%"
            );
        });

        it("returns the correct fee for 1005000", async () => {
            expect(await nftDescriptor.feeToPercentString(1005000)).to.eq(
                "100.5%"
            );
        });

        it("returns the correct fee for 10000000", async () => {
            expect(await nftDescriptor.feeToPercentString(10000000)).to.eq(
                "1000%"
            );
        });

        it("returns the correct fee for 12300000", async () => {
            expect(await nftDescriptor.feeToPercentString(12300000)).to.eq(
                "1230%"
            );
        });
    });
    ...
    describe("#tokenToColorHex", () => {
        ...
        it("returns the correct hash for the first 3 bytes of the token address", async () => {
            expect(
                await nftDescriptor.tokenToColorHex(tokens[0].address, 136)
            ).to.eq(tokenToColorHex(tokens[0].address, 2));
            expect(
                await nftDescriptor.tokenToColorHex(tokens[1].address, 136)
            ).to.eq(tokenToColorHex(tokens[1].address, 2));
        });

        it("returns the correct hash for the last 3 bytes of the address", async () => {
            expect(
                await nftDescriptor.tokenToColorHex(tokens[0].address, 0)
            ).to.eq(tokenToColorHex(tokens[0].address, 36));
            expect(
                await nftDescriptor.tokenToColorHex(tokens[1].address, 0)
            ).to.eq(tokenToColorHex(tokens[1].address, 36));
        });
    });

    describe("#rangeLocation", () => {
        it("returns the correct coordinates when range midpoint under -125_000", async () => {
            const coords = await nftDescriptor.rangeLocation(
                -887_272,
                -887_100
            );
    ...
    describe("#isRare", () => {
        it("returns true sometimes", async () => {
            expect(await nftDescriptor.isRare(1, `0x${"b".repeat(40)}`)).to.eq(
                true
            );
        });

        it("returns false sometimes", async () => {
            expect(await nftDescriptor.isRare(2, `0x${"b".repeat(40)}`)).to.eq(
                false
            );
        });
    });
    ...
    function constructTokenMetadata(
        ...
        return {
            name: `Shadow CL - ${feeTier} - ${quoteTokenSymbol}/${baseTokenSymbol} - ${prices}`,
            description: `This NFT represents a concentrated liquidity position in a Shadow ${quoteTokenSymbol}-${baseTokenSymbol} pool. The owner of this NFT can modify or redeem the position.\n\
