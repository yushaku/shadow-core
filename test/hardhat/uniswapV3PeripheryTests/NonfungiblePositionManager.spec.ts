...
describe("NonfungiblePositionManager", () => {
    ...
    beforeEach("load fixture", async () => {
        ({ nft, factory, tokens, weth9, router } = await loadFixture(
            nftFixture
        ));
    });
    ...
    describe("#mint", () => {
        ...
        it("creates a token", async () => {
            ...
            expect(tickLower).to.eq(
                getMinTick(TICK_SPACINGS[FeeAmount.MEDIUM])
            );
            expect(tickUpper).to.eq(
                        ...
                        nft.interface.encodeFunctionData("mint", [
                            {
                                token0: token0.address,
                                token1: token1.address,
                                tickLower: getMinTick(
                                    TICK_SPACINGS[FeeAmount.MEDIUM]
                                ),
                                tickUpper: getMaxTick(
                                    TICK_SPACINGS[FeeAmount.MEDIUM]
                                ),
                                fee: FeeAmount.MEDIUM,
                                recipient: wallet.address,
    ...
    describe("#increaseLiquidity", () => {
        ...
        it("can be paid with ETH", async () => {
            ...
            const refundETHData = nft.interface.encodeFunctionData(
                "unwrapWETH9",
                [0, other.address]
            );
    ...
    describe("#permit", () => {
        ...
        describe("owned by eoa", () => {
            ...
            it("changes the operator of the position and increments the nonce", async () => {
                ...
                expect((await nft.positions(tokenId)).operator).to.eq(
                    wallet.address
                );
            });
            ...
            it("cannot be called twice with the same signature", async () => {
                ...
                await nft.permit(wallet.address, tokenId, 1, v, r, s);
                await expect(nft.permit(wallet.address, tokenId, 1, v, r, s)).to
                    .be.reverted;
            });
    ...
    describe("multicall exit", () => {
        ...
        async function exit({
            ...
            const burnData = nft.interface.encodeFunctionData("burn", [
                tokenId,
            ]);

            return nft.multicall([
