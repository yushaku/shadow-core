...
import { OracleTest } from "./../../typechain-types/contracts/v2-periphery/test/OracleTest";
...
describe("OracleLibrary", () => {
    ...
    const oracleTestFixture = async () => {
        ...
        const oracleFactory = await ethers.getContractFactory(
            "contracts/v2-periphery/test/OracleTest.sol:OracleTest"
        );
    ...
    describe("#consult", () => {
        ...
        before("create mockObservableFactory", async () => {
            mockObservableFactory = await ethers.getContractFactory(
                "MockObservable"
            );
        });

        it("reverts when period is 0", async () => {
            await expect(oracle.consult(oracle.address, 0)).to.be.revertedWith(
                "BP"
            );
        });
        ...
        it("correct output when tick is 0", async () => {
            const period = 3;
            const secondsPerLiqCumulatives: [BigNumberish, BigNumberish] = [
                10, 20,
            ];
            ...
            const { arithmeticMeanTick, harmonicMeanLiquidity } =
                await oracle.callStatic.consult(mockObservable.address, period);
        ...
        function calculateHarmonicAvgLiq(
            ...
            const [secondsPerLiq0, secondsPerLiq1] =
        ...
        function observableWith({
            ...
            tickCumulatives,
            secondsPerLiqCumulatives,
            secondsPerBoostedLiqPeriod,
        }: {
            period: number;
            tickCumulatives: [BigNumberish, BigNumberish];
            secondsPerLiqCumulatives: [BigNumberish, BigNumberish];
            secondsPerBoostedLiqPeriod?: [BigNumberish, BigNumberish];
        }) {
            const _secondsPerBoostedLiqPeriod = secondsPerBoostedLiqPeriod ?? [
                0, 0,
            ];
            return mockObservableFactory.deploy(
                ...
                secondsPerLiqCumulatives.map(BigNumber.from),
                _secondsPerBoostedLiqPeriod?.map(BigNumber.from)
            );
        }
    });
    ...
    describe("#getBlockStartingTickAndLiquidity", () => {
        ...
        it("reverts if the pool is not initialized", async () => {
            ...
            await expect(
                oracle.getBlockStartingTickAndLiquidity(
                    mockObservations.address
                )
    ...
    describe("#getChainedPrice", () => {
        ...
        it("fails with discrepant length", async () => {
            ...
            expect(
                oracle.getChainedPrice(tokenAddresses, ticks)
        ...
        it("add two positive ticks, sorted order", async () => {
            ...
            ticks = [5, 5];
            const oracleTick = await oracle.getChainedPrice(
                tokenAddresses,
                ticks
            );
