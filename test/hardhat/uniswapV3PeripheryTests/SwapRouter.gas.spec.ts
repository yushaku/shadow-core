...
describe("SwapRouter gas tests", function () {
    ...
    const swapRouterFixture: () => Promise<{
        ...
        for (const poolAddress of poolAddresses) {
            _pools.push(
                await ethers.getContractAt("IClPool", poolAddress)
            );
        }

        // @ts-ignore: _pools to pools
        const pools: [IClPool, IClPool, IClPool] =
            _pools;
    ...
    beforeEach("load fixture", async () => {
        ({ router, weth9, tokens, pools } = await loadFixture(
            swapRouterFixture
        ));
    });
    ...
    describe("#exactOutput", () => {
        ...
        it("0 -> 1 -> 2", async () => {
            await snapshotGasCost(
                exactOutput(tokens.map((token) => token.address))
            );
        });
