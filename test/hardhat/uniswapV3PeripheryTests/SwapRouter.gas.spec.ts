...
import { IClPool, IWETH9, MockTimeSwapRouter } from "./../../typechain-types";
...
describe("SwapRouter gas tests", function () {
  ...
  const swapRouterFixture: () => Promise<{
    ...
    for (const poolAddress of poolAddresses) {
      _pools.push(await ethers.getContractAt("IClPool", poolAddress));
    }

    // @ts-ignore: _pools to pools
    const pools: [IClPool, IClPool, IClPool] = _pools;
  ...
  describe("#exactInput", () => {
    ...
    it("0 -> 1 minimal", async () => {
      const calleeFactory = await ethers.getContractFactory("TestClPoolCallee");
