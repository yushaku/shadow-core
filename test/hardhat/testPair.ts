...
describe("Pair", () => {
  ...
  it("swap:gas", async () => {
    ...
    await time.setNextBlockTimestamp((await wallet.provider.getBlock("latest"))!.timestamp + 1);
    await pair.sync();
  ...
  it("feeTo:on", async () => {
    ...
    expect(await token0.balanceOf(await pair.getAddress())).to.eq(1000n + 1497010102184673n);
    expect(await token1.balanceOf(await pair.getAddress())).to.eq(1000n + 1500001123877809n);
  });
});
