...
describe("Shadow", function () {
  async function deploy() {
    ...
    const shadow = (await ethers.deployContract("Shadow", [deployer.address])) as Shadow;
  ...
  it("Should not allow unauthorized addresses to mint", async function () {
    ...
    const amount = ethers.parseEther("100");
    await expect(shadow.connect(user).mint(user.address, amount)).to.be.reverted;
  });
  ...
  it("Should handle change of minter properly", async function () {
    ...
    await expect(shadow.connect(user).mint(user.address, amount)).to.not.be.reverted;
  ...
  it("Should transfer tokens properly", async function () {
    ...
    await expect(shadow.transfer(user.address, amount)).to.changeTokenBalances(
      shadow,
      [deployer, user],
      [-amount, amount],
    );
    ...
    await expect(shadow.connect(user).transfer(deployer.address, amount)).to.changeTokenBalances(
      shadow,
      [user, deployer],
      [-amount, amount],
    );
  ...
  it("Should not allow transferFrom without approval", async function () {
    ...
    const amount = ethers.parseEther("100");
    await shadow.mint(deployer.address, amount);
    await expect(shadow.connect(user).transferFrom(deployer.address, user.address, amount)).to.be
      .reverted;
  });
  ...
  it("Should allow transferFrom with approvals", async function () {
    ...
    expect(await shadow.allowance(deployer.address, user.address)).to.equal(amount);
    expect(await shadow.connect(user).transferFrom(deployer.address, user.address, amount));
    expect(await shadow.balanceOf(deployer.address)).to.equal(0);
    expect(await shadow.balanceOf(user.address)).to.equal(amount);
    expect(await shadow.allowance(deployer.address, user.address)).to.equal(0);
  });
});
