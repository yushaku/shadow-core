...
describe("Factories", function () {
  ...
  before(async () => {
    ...
    await c.pairFactory.createPair(c.usdc.getAddress(), c.weth.getAddress(), false);
    pairTwo = await ethers.getContractAt(
      "Pair",
      await c.pairFactory.getPair(c.usdc.getAddress(), c.weth.getAddress(), false),
    );
    await c.voter.createGauge(pairTwo.getAddress());
    gaugeTwo = await ethers.getContractAt("Gauge", await c.voter.gauges(pairTwo.getAddress()));
  ...
  it("Should return correct pair address in pairFor", async function () {
    expect(await c.router.pairFor(c.shadow.getAddress(), c.weth.getAddress(), false)).equal(
      await pair.getAddress(),
    );
    expect(await c.router.pairFor(c.usdc.getAddress(), c.weth.getAddress(), false)).equal(
      await pairTwo.getAddress(),
    );
  });
  ...
  it("Should have a new address for each new feeDistributor and gauge", async function () {
    ...
    expect(await feeDistributor.getAddress()).not.equal(await feeDistributorTwo.getAddress());
  });

  it("Should store the correct gauge and feeDistributor addresses", async function () {
    expect(await gauge.getAddress()).equal(await c.voter.gauges(pair.getAddress()));
    expect(await feeDistributor.getAddress()).equal(
      await c.voter.feeDistributors(gauge.getAddress()),
    );
    expect(await gaugeTwo.getAddress()).equal(await c.voter.gauges(pairTwo.getAddress()));
