...
contract x33Test is TheTestBase {
	...
	function test_claimIncentives() public {
		...
		vm.mockCall(
			...
			abi.encodeWithSelector(
				voter.claimIncentives.selector,
				address(TREASURY),
				distributors,
				tokens
			),
	...
	function testFuzz_swapIncentiveViaAggregator(uint256 amountIn, uint256 minAmountOut) public {
		...
		deal(address(xYSK), address(this), 1e18);
		xYSK.approve(address(vault), 1e18);
