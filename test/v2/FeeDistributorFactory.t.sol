...
contract FeeDistributorFactoryTest is TheTestBase {
	...
	function test_createFeeDistributor() public {
		...
		assertEq(
			factory.lastFeeDistributor(),
			newDistributor,
			"Last fee distributor not updated correctly"
		);
	...
	function test_createMultipleFeeDistributors() public {
		...
		assertTrue(
			...
			"Distributors should have different addresses"
		);
		...
		assertEq(
			factory.lastFeeDistributor(),
			secondDistributor,
			"Last fee distributor should be the second one"
		);
		...
		assertEq(
			distributor1.voter(),
			address(mockVoter),
			"First distributor voter not set correctly"
		);
		assertEq(
			distributor1.feeRecipient(),
			address(feeRecipient),
			"First distributor fee recipient not set correctly"
		);
		assertEq(
			distributor1.voteModule(),
			mockVoter.voteModule(),
			"First distributor vote module not set correctly"
		);
		assertEq(
			address(distributor1.plugin()),
			mockVoter.launcherPlugin(),
			"First distributor plugin not set correctly"
		);
		assertEq(
			distributor1.firstPeriod(),
			firstTimestamp / 1 weeks,
			"First distributor period not set correctly"
		);
		assertEq(
			distributor1.getPeriod(),
			(firstTimestamp + 2 weeks) / 1 weeks,
			"Current period calculation incorrect"
		);
		...
		assertEq(
			distributor2.voter(),
			address(mockVoter),
			"Second distributor voter not set correctly"
		);
		assertEq(
			distributor2.feeRecipient(),
			secondRecipient,
			"Second distributor fee recipient not set correctly"
		);
		assertEq(
			distributor2.voteModule(),
			mockVoter.voteModule(),
			"Second distributor vote module not set correctly"
		);
		assertEq(
			address(distributor2.plugin()),
			mockVoter.launcherPlugin(),
			"Second distributor plugin not set correctly"
		);
		assertEq(
