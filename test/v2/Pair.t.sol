...
contract PairTest is TheTestBase {
	Pair public pair;
	...
	function test_currentCumulativePrices() public {
		...
		(uint256 reserve0Cumulative, uint256 reserve1Cumulative, uint256 blockTimestamp) = pair
	...
	function test_stablePair() public {
		...
		Pair stablePair = Pair(
			pairFactory.createPair(address(token0Sorted), address(token1Sorted), true)
		);
...
contract MockCalleeWithCallback is TheTestBase {
